...
/**
   ...
   */
template <class T> inline bool is_power2(T val)
{
    T next = 1u;
...
/**
   ...
   */
template <class T> inline T upper_power2(T val)
{
    T next = 1u;
    for (unsigned int i = 0u; i < 32u; ++i)
    {
        if (next >= val) return next;
        next = next << 1u;
    }
    return 0u;
}
...
/**
   ...
   * and returns the value 2^n.
   */
template <class T> inline T lower_power2(T val)
{
