...
public:
    ...
    void load_config_file(std::string const& cfg_file)
    {
        ...
        bool const tetgen_suppress_splitting
            = util::to_value<bool>(m_config_file.get_value("tetgen_suppress_splitting", "true"));
        ...
        if (scene_name.compare("default") == 0) { soft_body::make_default_scene(m_engine, m_config_file); }
        else if (scene_name.compare("cantilever_tower") == 0)
        {
            soft_body::make_cantilever_tower_scene(m_engine, m_config_file);
        }
        else if (scene_name.compare("plate_stack") == 0) { soft_body::make_plate_stack_scene(m_engine, m_config_file); }
        else
        {
            util::Log logging;

            logging << "Application::load_config_file(): ERROR no such scene exist : " << scene_name
                    << util::Log::newline();
        }
    }
    ...
    int run()
    {
        ...
        while (m_time < m_total_time)
        {
            ...
            if (m_write_obj_files) soft_body::write_meshes(m_engine, m_output_path, m_frame_number);
        ...
        if (!m_did_auto_save)
        {
            if (m_profiling) { hyper::write_profiling(m_output_path + m_matlab_file); }
...

        {
            ...
            logging << "Application::run(): Total time reached = " << m_total_time
                    << " seconds, no more simulation is done... reload to restart" << util::Log::newline();
        }
