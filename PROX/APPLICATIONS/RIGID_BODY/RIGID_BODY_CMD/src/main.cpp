...
public:
    ...
    void save_contact_data()
    {
        ...
        filename << m_output_path << m_procedural_scene << "contact_data_" << std::setw(width) << std::setfill('0')
                 << counter << ".m";

        m_engine.write_contact_data(filename.str(), counter);
    ...
    int run()
    {
        ...
        while (m_time < m_total_time)
        {
            ...
            if (m_save_contact_data) { save_contact_data(); }
        }

        {
            util::Log logging;

            logging << "Application::run(): Total time reached " << m_total_time << util::Log::newline();
        }

        if (m_profiling) { m_engine.write_profiling(m_output_path + m_matlab_file); }

        if (m_xml_record) { save_xml_file(); }
