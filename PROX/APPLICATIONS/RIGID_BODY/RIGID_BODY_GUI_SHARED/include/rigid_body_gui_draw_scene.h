#ifndef RIGID_BODY_GUI_DRAW_SCENE_H
#define RIGID_BODY_GUI_DRAW_SCENE_H
  ...
  namespace gui
  {
                           ...
                           )
    {
      ...
      for(;object!=end;++object)
      {
        ...
        /*
         gl3::Texture3D const & texture = texture_manager.get( object->m_tid );

         glActiveTexture( GL_TEXTURE0 );            // Active texture unit 0
         gl3::check_errors("glActiveTexture( GL_TEXTURE0 );");
         texture.bind();                            // Bind this texture to texture unit 0
         program.set_uniform( "solid_texture", 0 ); // Solid sampler should use texture unit 0
         */
        set_material_uniforms(program, materials[object->m_tid]);
