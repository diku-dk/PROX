...
#include <rigid_body_gui_chalk_application.h>
...
QSize Widget::sizeHint() const { return QSize(1024, 768); }

void Widget::animate()
{
    bool finished = !Instance::app().run();
    update();
    if (finished) { m_timer.stop(); }
}
...
void Widget::initializeGL()
{
    ...
    if (GLEW_OK != err)
    {
        ...
        logging << "GLEW Error: " << glewGetErrorString(err) << util::Log::newline();
...
void Widget::save_screen(QString const& filename) { this->grabFrameBuffer().save(filename); }
