#ifndef RIGID_BODY_GUI_CHALK_APPLICATION_H
#define RIGID_BODY_GUI_CHALK_APPLICATION_H

#include <rigid_body_gui_chalk_write_grains.h>
#include <rigid_body_gui_chalk_make_procedural_content.h>
...
#include <rigid_body_gui_chalk_widget.h>
    ...
    namespace chalk
    {
      ...
      public:
        ...
        void save_xml_file()
        {
          ...
          logging << "Saved XML data to " << m_xml_save_scene_file << " and " << m_xml_save_channel_file << util::Log::newline();
        }
        ...
        void load_config_file(std::string const & cfg_file)
        {
          ...
          size_t total_vertices = 0, total_tetrahedrons = 0, total_bodies = 0;
          for(; total_bodies < m_engine.get_number_of_rigid_bodies(); ++total_bodies)
          {
            size_t geom_id = m_engine.get_rigid_body_collision_geometry(total_bodies);
            size_t N, K;
          ...
          logging << "chalk::Application::load_config_file(): "
          << total_bodies
          << " bodies, "
          << total_tetrahedrons
          << " tetrahedrons, "
        ...
        void save_contact_data()
        {
          ...
          m_engine.write_contact_data(filename.str(), 0u);
        }
      ...
      public:
        ...
        void paint()
        {
          ...
          // glm::mat4 const model_matrix      = glm::mat4(1.0f);
          glm::mat4 const view_matrix       = compute_view_matrix();
          glm::mat4 const projection_matrix = compute_projection_matrix();

          // glm::mat4 const MVP = projection_matrix * view_matrix * model_matrix;
          // m_frustum.update(MVP);
        ...
        void key_down(unsigned char choice)
        {
          ...
          switch(choice)
          {
            case '0': load_config_file(m_working_directory + "chalk.cfg");               break;
        ...
        bool run()
        {
          if (m_time >= m_total_time)
          {
            ...
            if(! m_did_auto_save)
            {
              ...
              write_grains("chalk/output.txt", &m_engine);
...
// RIGID_BODY_GUI_CHALK_APPLICATION_H
#endif
