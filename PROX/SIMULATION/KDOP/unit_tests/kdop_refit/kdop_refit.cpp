...
BOOST_AUTO_TEST_CASE(kdop_refit)
{
    ...
    T const height = 1.0f;
    T const depth = 2.0f;
    mesh_array::make_box<T>(width, height, depth, surface, sX, sY, sZ);
    ...
    kdop::Tree<T, 8> tree = kdop::make_tree<8, T>(32000, mesh_out, X_out, Y_out,
                                                  Z_out, kdop::sequential());

    kdop::refit_tree<8, T>(tree, mesh_out, X_out, Y_out, Z_out,
                           kdop::sequential());
}
