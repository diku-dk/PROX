...
/*      auto sdf_bbox = sdf.getBoundingBox();
  ...
  template <size_t K, typename T>
  inline void traversal_sdf_CCD_WARM_START(
      size_t const& node_idx_A, SubTree<T, K> const& branch_A,
      mesh_array::T4Mesh const& mesh_A,
      mesh_array::VertexAttribute<T, mesh_array::T4Mesh> const& X_A,
      mesh_array::VertexAttribute<T, mesh_array::T4Mesh> const& Y_A,
      ...
      geometry::ContactsCallback<T>& callback, std::vector<T>& TOIs,
      T startTime, T endTime, kdop::BodyVelocities<T>& bodyContact,
      size_t rigidId, std::vector<T>& warmBodyStart, size_t& triIdx,
      bool hasRestartWarmBodies)
  {
      using namespace mesh_array;
      ...
      if (A_is_leaf)
      {
          ...
          if (!hasRestartWarmBodies) { startTime = warmBodyStart[triIdx]; }
          if (surface_A[0])
          { // Face opposite vertex i (vertices j,k,m)
              /*EigenVector3<T> contactPoint;
              EigenVector3<T> normal;
          ...
          //Only process TOI warmstarter if any tri found!
          if (surfaceFound)
          {
              if (hasRestartWarmBodies)
              {
                  warmBodyStart.push_back(TOIs.back());
              }
              else { warmBodyStart[triIdx] = TOIs.back(); }
              triIdx += 1;
          }
          STOP_TIMER("CCD_TOTAL_TOI_TIME_ONE_TRIANGLE");
      ...
      else if (!A_is_leaf)
      {
          ...
          for (size_t a = node_A.m_start; a <= node_A.m_end; ++a)
          {
              traversal_sdf_CCD_WARM_START<K, T>(
                  a, branch_A, mesh_A, X_A, Y_A, Z_A, surface_map_A, sdf,
                  transformTranslation, transformRotation, directions, callback,
                  TOIs, startTime, endTime, bodyContact, rigidId, warmBodyStart,
                  triIdx, hasRestartWarmBodies);
          }
      }
  }
  ...
  // Top-level function for SDF vs TetraMesh collision
  template <size_t K, typename T>
  inline void tandem_traversal_sdf_CCD_WARM_START(
      kdop::TestPairSDFStruct<K, T>& work_item, std::vector<T>& TOIs,
      T startTime, T endTime, kdop::BodyVelocities<T>& bodyContact,
      size_t rigidId, std::vector<T>& warmBodyStart)
  {
      if (!work_item.m_tree_a || !work_item.m_grid_b) return;
      ...
      bool hasRestartWarmBodies = false;
      if (warmBodyStart.empty()) { hasRestartWarmBodies = true; }
      // Process all branches
      for (auto const& branch : work_item.m_tree_a->branches())
      {
          ...
          traversal_sdf_CCD_WARM_START<K, T>(
              ...
              *(work_item.m_transformRotation_b), directions,
              *(work_item.m_callback), TOIs, startTime, endTime, bodyContact,
              rigidId, warmBodyStart, triIdx, hasRestartWarmBodies);
      }
  }
      ...
      std::vector<std::vector<T>>& warmBodiesStart)
  {
      ...
      bool restartWarmBodies = false;
      if (warmBodiesStart.size() != work_pool.size())
      {
          warmBodiesStart.clear();
          restartWarmBodies = true;
      }
      for (size_t i = 0; i < work_pool.size(); ++i)
      {
          if (restartWarmBodies)
          {
              START_TIMER("CCD_TOTAL_TOI_TIME_ONE_BODY");
              {
                  std::vector<T> warmBody;
                  tandem_traversal_sdf_CCD_WARM_START<K, T>(
                      work_pool[i], TOIs, startTime, endTime, bodyContacts[i],
                      i, warmBody);
                  warmBodiesStart.push_back(warmBody);
              }
              STOP_TIMER("CCD_TOTAL_TOI_TIME_ONE_BODY");
          }
          else
