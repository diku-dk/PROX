...
ProxEngine::ProxEngine(std::string const& solver, std::string const& normal_solver, std::string const& friction_solver,
                       std::string const& time_stepper, std::string const& r_factor_strategy)
{
    m_data = new ProxData();
...
void ProxEngine::set_rigid_body_position(size_t const& body_idx, float const& x, float const& y, float const& z)
{
...
void ProxEngine::set_rigid_body_inertia(size_t const& body_idx, float const& Ixx, float const& Iyy, float const& Izz)
{
    assert(m_data || !"internal error: null pointer");
    assert(body_idx < m_data->m_bodies.size() || !"internal error: no such rigid body");
...
void ProxEngine::connect_collision_geometry(size_t body_idx, size_t geometry_index)
{
    ...
    if (geometry.m_tetramesh.has_data()) { narrow::make_kdop_bvh(m_data->m_narrow.params(), body, geometry); }
}
...
size_t ProxEngine::create_material(std::string const& name)
{
    ...
    assert(idx < m_number_of_materials
           || !"internal error: more materials created than is allocated space for in material table");
    return idx;
}

void ProxEngine::create_material_property(size_t const& first_idx, size_t const& second_idx)
{
    assert(m_data || !"internal error: null pointer");
    assert(first_idx < m_number_of_materials || !"internal error: index excedes allocated space for in material table");
    assert(second_idx < m_number_of_materials
           || !"internal error: index excedes allocated space for in material table");
...
void ProxEngine::set_master(size_t const& first_idx, size_t const& second_idx, size_t master_idx)
{
    assert(m_data || !"internal error: null pointer");
    assert(first_idx < m_number_of_materials || !"internal error: index excedes allocated space for in material table");
    assert(second_idx < m_number_of_materials
           || !"internal error: index excedes allocated space for in material table");
    assert(master_idx < m_number_of_materials
           || !"internal error: index excedes allocated space for in material table");
...
void ProxEngine::set_friction(size_t const& first_idx, size_t const& second_idx, float const& mu_x, float const& mu_y,
                              float const& mu_z)
{
    assert(m_data || !"internal error: null pointer");
    assert(first_idx < m_number_of_materials || !"internal error: index excedes allocated space for in material table");
...
void ProxEngine::set_master_direction(size_t const& first_idx, size_t const& second_idx, float const& dir_x,
                                      float const& dir_y, float const& dir_z)
{
    assert(m_data || !"internal error: null pointer");
    assert(first_idx < m_number_of_materials || !"internal error: index excedes allocated space for in material table");
    assert(second_idx < m_number_of_materials
...
void ProxEngine::set_restitution(size_t const& first_idx, size_t const& second_idx, float const& e)
{
    assert(m_data || !"internal error: null pointer");
    assert(first_idx < m_number_of_materials || !"internal error: index excedes allocated space for in material table");
    assert(second_idx < m_number_of_materials
...
size_t ProxEngine::create_box_shape(size_t const& geometry_index)
{
    ...
    if (m_data->m_use_only_tetrameshes)
    {
        assert(!geometry.m_tetramesh.has_data()
               || !"create_box_shape(): Internal error, tetramesh shape already created");
        return 0u;
    }
...
void ProxEngine::set_box_shape(size_t const& geometry_index, size_t const& box_number, float const& width,
...
size_t ProxEngine::create_capsule_shape(size_t const& geometry_index)
{
    ...
    assert(geometry.m_tetramesh.has_data()
           || !"create_capsule_shape(): Internal error, tetramesh shape already created");

    return 0;
}

void ProxEngine::set_capsule_shape(size_t const& geometry_index, size_t const& capsule_number, float const& radius,
                                   float const& height)
{
    assert(m_data || !"internal error: null pointer");
    ...
    mesh_array::make_capsule<T>(radius, height, slices, segments, surface, surface_X, surface_Y, surface_Z);

    m_data->make_tetramesh_geoemtry(geometry, surface, surface_X, surface_Y, surface_Z);
}

void ProxEngine::set_capsule_position(size_t const& geometry_index, size_t const& capsule_number, float const& x,
                                      float const& y, float const& z)
{
    assert(!"not implemented");
}

void ProxEngine::set_capsule_orientation(size_t const& geometry_index, size_t const& capsule_number, float const& Qs,
                                         float const& Qx, float const& Qy, float const& Qz)
{
    assert(!"not implemented");
}
...
void ProxEngine::set_cone_shape(size_t const& geometry_index, size_t const& cone_number, float const& radius,
                                float const& height)
{
    assert(m_data || !"internal error: null pointer");
    assert((is_number(radius) && is_number(height)) || !"internal error: NaN or inf value");
    ...
    mesh_array::make_cone<T>(radius, height, slices, surface, surface_X, surface_Y, surface_Z);

    m_data->make_tetramesh_geoemtry(geometry, surface, surface_X, surface_Y, surface_Z);
}

void ProxEngine::set_cone_position(size_t const& geometry_index, size_t const& cone_number, float const& x,
                                   float const& y, float const& z)
{
    assert(!"not implemented");
}

void ProxEngine::set_cone_orientation(size_t const& geometry_index, size_t const& cone_number, float const& Qs,
                                      float const& Qx, float const& Qy, float const& Qz)
{
    assert(!"not implemented");
}
...
void ProxEngine::set_convex_shape(size_t const& geometry_index, size_t const& convex_number, size_t const& N,
...
/*      p[i](0) = coordinates[3u*i+0u];
    ...
    if (m_data->m_use_only_tetrameshes)
    {
        ...
        mesh_array::make_convex<T>(p, surface, surface_X, surface_Y, surface_Z);
    ...
    for (size_t i = 0u; i < p.size(); ++i) { geometry.m_hulls[convex_number].data().add_point((p[i])); }
}

void ProxEngine::set_convex_position(size_t const& geometry_index, size_t const& convex_number, float const& x,
                                     float const& y, float const& z)
{
...
size_t ProxEngine::create_cylinder_shape(size_t const& geometry_index)
{
    ...
    // So idea is to use tetramesh to fake a convex shape
    assert(geometry.m_tetramesh.has_data()
           || !"create_cylinder_shape(): Internal error, tetramesh shape already created");

    return 0u;
}

void ProxEngine::set_cylinder_shape(size_t const& geometry_index, size_t const& cylinder_number, float const& radius,
                                    float const& height)
{
    assert(m_data || !"internal error: null pointer");
    assert((is_number(radius) && is_number(height)) || !"internal error: NaN or inf value");
    ...
    mesh_array::make_cylinder<T>(radius, height, slices, surface, surface_X, surface_Y, surface_Z);

    m_data->make_tetramesh_geoemtry(geometry, surface, surface_X, surface_Y, surface_Z);
}

void ProxEngine::set_cylinder_position(size_t const& geometry_index, size_t const& cylinder_number, float const& x,
                                       float const& y, float const& z)
{
    assert(!"not implemented");
}

void ProxEngine::set_cylinder_orientation(size_t const& geometry_index, size_t const& cylinder_number, float const& Qs,
                                          float const& Qx, float const& Qy, float const& Qz)
{
    assert(!"not implemented");
}
...
size_t ProxEngine::create_ellipsoid_shape(size_t const& geometry_index)
{
    ...
    // So idea is to use tetramesh to fake a convex shape
    assert(geometry.m_tetramesh.has_data()
           || !"create_ellipsoid_shape(): Internal error, tetramesh shape already created");

    return 0u;
}

void ProxEngine::set_ellipsoid_shape(size_t const& geometry_index, size_t const& ellipsoid_number, float const& sx,
                                     float const& sy, float const& sz)
{
    ...
    mesh_array::make_ellipsoid(sx, sy, sz, slices, segments, surface, surface_X, surface_Y, surface_Z);

    m_data->make_tetramesh_geoemtry(geometry, surface, surface_X, surface_Y, surface_Z);
}

void ProxEngine::set_ellipsoid_position(size_t const& geometry_index, size_t const& ellipsoid_number, float const& x,
                                        float const& y, float const& z)
{
    assert(!"not implemented");
}

void ProxEngine::set_ellipsoid_orientation(size_t const& geometry_index, size_t const& ellipsoid_number,
                                           float const& Qs, float const& Qx, float const& Qy, float const& Qz)
{
    assert(!"not implemented");
}
...
void ProxEngine::set_sphere_shape(size_t const& geometry_index, size_t const& sphere_number, float const& radius)
{
    assert(m_data || !"internal error: null pointer");
    ...
    if (m_data->m_use_only_tetrameshes)
    {
        ...
        mesh_array::make_sphere<T>(radius, slices, segments, surface, surface_X, surface_Y, surface_Z);
...
void ProxEngine::set_sphere_position(size_t const& geometry_index, size_t const& sphere_number, float const& x,
                                     float const& y, float const& z)
{
    assert(m_data || !"internal error: null pointer");
    assert((is_number(x) && is_number(y) && is_number(z)) || !"internal error: NaN or inf value");
...
{
    assert(m_data || !"internal error: null pointer");
    for (size_t i = 0; i < m_data->m_materials.size(); ++i) { index_array[i] = i; }
}
...
void ProxEngine::get_material_property_indices(size_t* first_index_array, size_t* second_index_array)
{
    assert(m_data || !"internal error: null pointer");
...
size_t ProxEngine::get_master(size_t const& first_index, size_t const& second_index)
{
    assert(m_data || !"internal error: null pointer");
    assert(first_index < m_number_of_materials
           || !"internal error: index excedes allocated space for in material table");
    assert(second_index < m_number_of_materials
           || !"internal error: index excedes allocated space for in material table");

    return m_data->m_properties[first_index][second_index].get_master_material_idx();
}

void ProxEngine::get_friction(size_t const& first_index, size_t const& second_index, float& x, float& y, float& z)
{
...
void ProxEngine::get_master_direction(size_t const& first_index, size_t const& second_index, float& x, float& y,
                                      float& z)
{
    assert(m_data || !"internal error: null pointer");
    assert(first_index < m_number_of_materials
...
float ProxEngine::get_restitution(size_t const& first_index, size_t const& second_index)
{
    assert(m_data || !"internal error: null pointer");
    assert(first_index < m_number_of_materials
           || !"internal error: index excedes allocated space for in material table");
...
void ProxEngine::get_geometry_indices(size_t* index_array)
{
    ...
    for (size_t i = 0; i < m_data->m_narrow.size(); ++i) { index_array[i] = i; }
}
...
void ProxEngine::get_box_shape(size_t const& geometry_index, size_t const& box_number, float& width, float& height,
                               float& depth)
{
    assert(m_data || !"internal error: null pointer");
...
void ProxEngine::get_box_position(size_t const& geometry_index, size_t const& box_number, float& x, float& y, float& z)
{
    assert(m_data || !"internal error: null pointer");
...
void ProxEngine::get_box_orientation(size_t const& geometry_index, size_t const& box_number, float& Qs, float& Qx,
                                     float& Qy, float& Qz)
{
    assert(m_data || !"internal error: null pointer");
...
void ProxEngine::get_cone_shape(size_t const& geometry_index, size_t const& cone_number, float& radius, float& height)
{
    assert(!"not implemented");
}

void ProxEngine::get_cone_position(size_t const& geometry_index, size_t const& cone_number, float& x, float& y,
                                   float& z)
{
    assert(!"not implemented");
}

void ProxEngine::get_cone_orientation(size_t const& geometry_index, size_t const& cone_number, float& Qs, float& Qx,
                                      float& Qy, float& Qz)
{
    assert(!"not implemented");
}

size_t ProxEngine::get_number_of_capsules(size_t const& geometry_index) { return 0u; }

void ProxEngine::get_capsule_shape(size_t const& geometry_index, size_t const& capsule_number, float& radius,
                                   float& height)
{
    assert(!"not implemented");
}

void ProxEngine::get_capsule_position(size_t const& geometry_index, size_t const& capsule_number, float& x, float& y,
                                      float& z)
{
    assert(!"not implemented");
}

void ProxEngine::get_capsule_orientation(size_t const& geometry_index, size_t const& capsule_number, float& Qs,
                                         float& Qx, float& Qy, float& Qz)
{
    assert(!"not implemented");
}

size_t ProxEngine::get_number_of_cylinders(size_t const& geometry_index) { return 0u; }

void ProxEngine::get_cylinder_shape(size_t const& geometry_index, size_t const& cylinder_number, float& radius,
                                    float& height)
{
    assert(!"not implemented");
}

void ProxEngine::get_cylinder_position(size_t const& geometry_index, size_t const& cylinder_number, float& x, float& y,
                                       float& z)
{
    assert(!"not implemented");
}

void ProxEngine::get_cylinder_orientation(size_t const& geometry_index, size_t const& cylinder_number, float& Qs,
                                          float& Qx, float& Qy, float& Qz)
{
    assert(!"not implemented");
}

size_t ProxEngine::get_number_of_ellipsoids(size_t const& geometry_index) { return 0u; }

void ProxEngine::get_ellipsoid_shape(size_t const& geometry_index, size_t const& ellipsoid_number, float& scale_x,
                                     float& scale_y, float& scale_z)
{
    assert(!"not implemented");
}

void ProxEngine::get_ellipsoid_position(size_t const& geometry_index, size_t const& ellipsoid_number, float& x,
                                        float& y, float& z)
{
    assert(!"not implemented");
}

void ProxEngine::get_ellipsoid_orientation(size_t const& geometry_index, size_t const& ellipsoid_number, float& Qs,
                                           float& Qx, float& Qy, float& Qz)
{
    assert(!"not implemented");
}
...
void ProxEngine::get_sphere_shape(size_t const& geometry_index, size_t const& sphere_number, float& radius)
{
    assert(m_data || !"internal error: null pointer");
...
void ProxEngine::get_sphere_position(size_t const& geometry_index, size_t const& sphere_number, float& x, float& y,
                                     float& z)
{
    assert(m_data || !"internal error: null pointer");
...
void ProxEngine::get_sphere_orientation(size_t const& geometry_index, size_t const& sphere_number, float& Qs, float& Qx,
                                        float& Qy, float& Qz)
{
    assert(m_data || !"internal error: null pointer");
...
void ProxEngine::get_tetramesh_shape(size_t const& geometry_index, size_t* vertices, size_t* tetrahedra,
                                     float* coordinates)
{
    assert(m_data || !"internal error: null pointer");
...
void ProxEngine::get_convex_shape(size_t const& geometry_index, size_t const& convex_number, size_t& no_points)
{
    assert(m_data || !"internal error: null pointer");
...
void ProxEngine::get_convex_shape(size_t const& geometry_index, size_t const& convex_number, float* coordinates)
{
    assert(m_data || !"internal error: null pointer");
...
void ProxEngine::get_convex_position(size_t const& geometry_index, size_t const& convex_number, float& x, float& y,
                                     float& z)
{
    assert(m_data || !"internal error: null pointer");
...
void ProxEngine::get_convex_orientation(size_t const& geometry_index, size_t const& convex_number, float& Qs, float& Qx,
                                        float& Qy, float& Qz)
{
    assert(m_data || !"internal error: null pointer");
...
{
    assert(m_data || !"internal error: null pointer");
    for (size_t i = 0; i < m_data->m_bodies.size(); ++i) { index_array[i] = i; }
}
...
void ProxEngine::get_rigid_body_position(size_t const& body_index, float& x, float& y, float& z)
{
    assert(m_data || !"internal error: null pointer");
    assert(body_index < m_data->m_bodies.size() || !"internal error: no such rigid body");
...
void ProxEngine::get_rigid_body_orientation(size_t const& body_index, float& Qs, float& Qx, float& Qy, float& Qz)
{
    assert(m_data || !"internal error: null pointer");
    assert(body_index < m_data->m_bodies.size() || !"internal error: no such rigid body");
...
void ProxEngine::get_rigid_body_spin(size_t const& body_index, float& x, float& y, float& z)
{
    assert(m_data || !"internal error: null pointer");
    assert(body_index < m_data->m_bodies.size() || !"internal error: no such rigid body");
...
void ProxEngine::get_rigid_body_inertia(size_t const& body_index, float& xx, float& yy, float& zz)
{
    assert(m_data || !"internal error: null pointer");
    assert(body_index < m_data->m_bodies.size() || !"internal error: no such rigid body");
...
void ProxEngine::get_rigid_body_bounding_box(size_t const& body_index, float& min_x, float& min_y, float& min_z,
...
void ProxEngine::set_gravity_up(float const& x, float const& y, float const& z)
{
    assert(m_data || !"internal error: null pointer");
...
void ProxEngine::get_gravity_up(float& x, float& y, float& z)
{
    assert(m_data || !"internal error: null pointer");
...
void ProxEngine::get_pin_force_indices(size_t* index_array)
{
    ...
    for (size_t i = 0; i < m_data->m_pin_forces.size(); ++i) { index_array[i] = m_data->m_pin_forces[i].get_idx(); }
}
...
void ProxEngine::set_scripted_key_position(size_t const& motion_index, float const& time, float const& x,
                                           float const& y, float const& z)
{
    assert(m_data || !"internal error: null pointer");
...
void ProxEngine::set_scripted_key_orientation(size_t const& motion_index, float const& time, float const& qs,
                                              float const& qx, float const& qy, float const& qz)
{
    assert(m_data || !"internal error: null pointer");
...
size_t ProxEngine::get_scripted_motion(size_t const& body_index)
{
    ...
    assert(lookup != m_data->m_motion_callbacks.end()
           || !"get_scripted_motion(): No scripted motion was connected to this body");
...
void ProxEngine::set_scripted_oscilation_paramters(size_t const& motion_index, float const& amplitude,
                                                   float const& frequency, float const& phase, float const& dir_x,
                                                   float const& dir_y, float const& dir_z, float const& ref_x,
                                                   float const& ref_y, float const& ref_z)
{
    ...
    assert(lookup != m_data->m_oscillation_motions.end()
           || !"set_scripted_oscilation_paramters(): Motion did not exist");
...
void ProxEngine::get_scripted_oscilation_paramters(size_t const& motion_index, float& amplitude, float& frequency,
                                                   float& phase, float& dir_x, float& dir_y, float& dir_z)
{
    ...
    assert(lookup != m_data->m_oscillation_motions.end()
           || !"get_scripted_oscilation_paramters(): motion was not oscilation scripted motion");
...
size_t ProxEngine::get_number_of_key_frame_positions(size_t const& motion_index)
{
    ...
    assert(lookup != m_data->m_keyframe_motions.end()
           || !"get_number_of_key_frame_positions(): motion was not keyframe scripted motion");
...
void ProxEngine::get_key_frame_positions(size_t const& motion_index, float* time_array, float* x_array, float* y_array,
                                         float* z_array)
{
...
void ProxEngine::get_key_frame_orientations(size_t const& motion_index, float* time_array, float* qs_array,
                                            float* qx_array, float* qy_array, float* qz_array)
{
    ...
    assert(lookup != m_data->m_keyframe_motions.end()
           || !"get_key_frame_orientations(): motion was not keyframe scripted motion");
...
bool ProxEngine::compute_raycast(float const& p_x, float const& p_y, float const& p_z, float const& ray_x,
                                 float const& ray_y, float const& ray_z, size_t& body_idx, float& hit_x, float& hit_y,
                                 float& hit_z, float& distance)
{
    assert(m_data || !"compute_raycast(): data was null pointer");

    return m_data->compute_raycast(p_x, p_y, p_z, ray_x, ray_y, ray_z, body_idx, hit_x, hit_y, hit_z, distance);
}
