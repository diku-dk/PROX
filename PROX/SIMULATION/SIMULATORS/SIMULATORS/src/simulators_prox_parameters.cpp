...
std::string const ProxEngine::PARAM_SOLVER = "solver";
std::string const ProxEngine::VALUE_JACOBI = "jacobi";
std::string const ProxEngine::VALUE_GAUSS_SEIDEL = "gauss_seidel";
std::string const ProxEngine::PARAM_NORMAL_SOLVER = "normal_sub_solver";
std::string const ProxEngine::VALUE_NONNEGATIVE = "nonnegative";
std::string const ProxEngine::VALUE_ORIGIN = "origin";
std::string const ProxEngine::VALUE_INFINITY = "infinity";
std::string const ProxEngine::PARAM_FRICTION_SOLVER = "friction_sub_solver";
std::string const ProxEngine::VALUE_ANALYTICAL_SPHERE = "analytical_sphere";
std::string const ProxEngine::VALUE_ANALYTICAL_ELLIPSOID = "analytical_ellipsoid";
std::string const ProxEngine::VALUE_NUMERICAL_ELLIPSOID = "numerical_ellipsoid";
std::string const ProxEngine::VALUE_GJK_ELLIPSOID = "gjk_ellipsoid";
std::string const ProxEngine::VALUE_BOX_MODEL = "box_model";
std::string const ProxEngine::PARAM_TIME_STEPPER = "time_stepper";
std::string const ProxEngine::VALUE_MOREAU = "moreau";
std::string const ProxEngine::VALUE_SEMI_IMPLICIT = "semi_implicit";
std::string const ProxEngine::VALUE_EMPTY = "empty";
std::string const ProxEngine::PARAM_R_FACTOR = "r_factor_strategy";
std::string const ProxEngine::VALUE_LOCAL = "local";
std::string const ProxEngine::VALUE_GLOBAL = "global";
std::string const ProxEngine::VALUE_BLOCK = "blocked";
std::string const ProxEngine::PARAM_CONTACT_ALGORITHM = "contact_algorithm";
std::string const ProxEngine::VALUE_SAT = "sat";
std::string const ProxEngine::VALUE_RESTRICTED = "restricted";
std::string const ProxEngine::VALUE_OPPOSING = "opposing";
std::string const ProxEngine::VALUE_INTERSECTION = "intersection";
std::string const ProxEngine::VALUE_VERTEX = "vertex";
std::string const ProxEngine::VALUE_CONSISTENT = "consistent";
std::string const ProxEngine::VALUE_GROWTH = "growth";
std::string const ProxEngine::VALUE_CLOSEST = "closest";
std::string const ProxEngine::PARAM_PRE_STABILIZATION = "pre_stabilization";
std::string const ProxEngine::PARAM_POST_STABILIZATION = "post_stabilization";
std::string const ProxEngine::PARAM_COTNACT_REDUCTION = "contact_reduction";
...
std::string const ProxEngine::PARAM_GAP_REDUCTION = "gap_reduction";
std::string const ProxEngine::PARAM_MAX_GAP = "max_gap";
std::string const ProxEngine::PARAM_MIN_GAP = "min_gap";
std::string const ProxEngine::PARAM_TETGEN_QUALITY_RATIO = "tetgen_quality_ratio";
std::string const ProxEngine::PARAM_TETGEN_MAXIMUM_VOLUME = "tetgen_maximum_volume";
std::string const ProxEngine::PARAM_NARROW_ENVELOPE = "narrow_envelope";
std::string const ProxEngine::PARAM_COLLISION_ENVELOPE = "collision_envelope";
std::string const ProxEngine::PARAM_TIME_STEP = "time_step";

std::string const ProxEngine::PARAM_BROAD_PHASE_ALGORITHM = "broad_phase_algorithm";
std::string const ProxEngine::VALUE_ALL_PAIR = "all_pair";
std::string const ProxEngine::VALUE_GRID = "grid";

void ProxEngine::set_parameter(std::string const& name, std::string const& value)
{
    assert(m_data || !"set_parameter(): internal error, null pointer");

    if (name == PARAM_SOLVER)
    {
        if (value == VALUE_JACOBI) { m_data->m_params.solver_params().set_solver(prox::jacobi); }
        else if (value == VALUE_GAUSS_SEIDEL) { m_data->m_params.solver_params().set_solver(prox::gauss_seidel); }
        else
        {
            util::Log logging;

            logging << "ProxEngine::set_parameter(): unknown solver value = " << value << util::Log::newline();
        }
    }
    else if (name == PARAM_NORMAL_SOLVER)
    {
        if (value == VALUE_NONNEGATIVE) { m_data->m_params.solver_params().set_normal_sub_solver(prox::nonnegative); }
        else if (value == VALUE_ORIGIN) { m_data->m_params.solver_params().set_normal_sub_solver(prox::normal_origin); }
        ...
        else
        {
            ...
            logging << "ProxEngine::set_parameter(): unknown normal sub solver value = " << value
                    << util::Log::newline();
        }
    }
    else if (name == PARAM_FRICTION_SOLVER)
    {
        ...
        else if (value == VALUE_GJK_ELLIPSOID)
        {
            m_data->m_params.solver_params().set_friction_sub_solver(prox::gjk_ellipsoid);
        }
        ...
        else
        {
            ...
            logging << "ProxEngine::set_parameter(): unknown friction sub solver value = " << value
                    << util::Log::newline();
        }
    }
    else if (name == PARAM_TIME_STEPPER)
    {
        if (value == VALUE_MOREAU) { m_data->m_params.stepper_params().set_stepper(prox::moreau); }
        else if (value == VALUE_SEMI_IMPLICIT) { m_data->m_params.stepper_params().set_stepper(prox::semi_implicit); }
        else if (value == VALUE_EMPTY) { m_data->m_params.stepper_params().set_stepper(prox::empty); }
        else
        {
            util::Log logging;

            logging << "ProxEngine::set_parameter(): unknown time stepper value = " << value << util::Log::newline();
        }
    }
    else if (name == PARAM_R_FACTOR)
    {
        if (value == VALUE_LOCAL) { m_data->m_params.solver_params().set_r_factor_strategy(prox::local_strategy); }
        ...
        else
        {
            ...
            logging << "ProxEngine::set_parameter(): unknown r-factor strategy value = " << value
                    << util::Log::newline();
        }
    }
    ...
    {
        if (value == VALUE_ALL_PAIR) { m_data->m_params.use_all_pair() = true; }
        else if (value == VALUE_GRID) { m_data->m_params.use_all_pair() = false; }
        else
        {
            util::Log logging;

            logging << "ProxEngine::set_parameter(): unknown broad phase algorithm = " << value << util::Log::newline();
        }
    }
    else
    {
        ...
        logging << "ProxEngine::set_parameter(string): unknown paramter name = " << name << util::Log::newline();
    }
}

void ProxEngine::set_parameter(std::string const& name, bool const& value)
{
    assert(m_data || !"set_parameter(bool): internal error, null pointer");

    if (name == PARAM_PRE_STABILIZATION) { m_data->m_params.stepper_params().set_pre_stabilization(value); }
    else if (name == PARAM_POST_STABILIZATION) { m_data->m_params.stepper_params().set_post_stabilization(value); }
    else if (name == PARAM_COTNACT_REDUCTION) { m_data->m_params.stepper_params().set_contact_reduction(value); }
    else if (name == PARAM_BOUNCE_ON) { m_data->m_params.stepper_params().set_bounce_on(value); }
    else if (name == PARAM_TETGEN_QUIET) { m_data->m_tetgen_settings.m_quiet_output = value; }
    else if (name == PARAM_TETGEN_SUPPRESS_SPLITTING) { m_data->m_tetgen_settings.m_suppress_splitting = value; }
    else if (name == PARAM_NARROW_USE_OPEN_CL) { m_data->m_narrow.params().set_use_open_cl(value); }
    else if (name == PARAM_NARROW_USE_GPROXIMITY) { m_data->m_narrow.params().set_use_gproximity(value); }
    else if (name == PARAM_NARROW_USE_BATCHING) { m_data->m_narrow.params().set_use_batching(value); }
    else if (name == PARAM_USE_ONLY_TETRAMESHES) { m_data->m_use_only_tetrameshes = value; }
    else
    {
        util::Log logging;

        logging << "ProxEngine::set_parameter(bool): unknown params name = " << name << util::Log::newline();
    }
}

void ProxEngine::set_parameter(std::string const& name, unsigned int const& value)
{
    assert(m_data || !"set_parameter(unsigned int): internal error, null pointer");

    if (name == PARAM_MAX_ITERATION) { m_data->m_params.solver_params().set_max_iterations(value); }
    else if (name == PARAM_NARROW_OPEN_CL_PLATFORM) { m_data->m_narrow.params().set_open_cl_platform(value); }
    else if (name == PARAM_NARROW_OPEN_CL_DEVICE) { m_data->m_narrow.params().set_open_cl_device(value); }
    else if (name == PARAM_NARROW_CHUNK_BYTES) { m_data->m_narrow.params().set_chunk_bytes(value); }
    else
    {
        util::Log logging;

        logging << "ProxEngine::set_parameter(unsigned int): unknown params name = " << name << util::Log::newline();
    }
}

void ProxEngine::set_parameter(std::string const& name, float const& value)
{
    assert(m_data || !"set_parameter(float): internal error, null pointer");

    if (name == PARAM_ABSOLUTE_TOLERANCE) { m_data->m_params.solver_params().set_absolute_tolerance(value); }
    else if (name == PARAM_RELATIVE_TOLERANCE) { m_data->m_params.solver_params().set_relative_tolerance(value); }
    else if (name == PARAM_GAP_REDUCTION) { m_data->m_params.stepper_params().set_gap_reduction(value); }
    else if (name == PARAM_MAX_GAP) { m_data->m_params.stepper_params().set_max_gap(value); }
    else if (name == PARAM_MIN_GAP) { m_data->m_params.stepper_params().set_min_gap(value); }
    else if (name == PARAM_TETGEN_QUALITY_RATIO) { m_data->m_tetgen_settings.m_quality_ratio = value; }
    else if (name == PARAM_TETGEN_MAXIMUM_VOLUME) { m_data->m_tetgen_settings.m_maximum_volume = value; }
    ...
    else
    {
        ...
        logging << "ProxEngine::set_parameter(float): unknown params name = " << name << util::Log::newline();
    }
}

void ProxEngine::set_parameters_from_config_file(std::string const& cfg_file)
{
    ...
    bool const tetgen_suppress_splitting
        = util::to_value<bool>(settings.get_value(PARAM_TETGEN_SUPPRESS_SPLITTING, "true"));
