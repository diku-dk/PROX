#include <mass.h>

#include <eigenhelperall.h>

#include <cassert>

namespace mass
{

template <typename T> Properties<T> compute_ellipsoid(T const& density, T const& a, T const& b, T const& c)
{

    assert(is_number(density) || !"density must be a number");
    assert(is_finite(density) || !"density must be a finite number");
    assert(density > 0 || !"density must be positive");
    assert(is_number(a) || !"a must be a  number");
    assert(is_finite(a) || !"a must be a finite number");
    assert(a > 0 || !"a must be positive");
    assert(is_number(b) || !"b must be a  number");
    assert(is_finite(b) || !"b must be a finite number");
    assert(b > 0 || !"b must be positive");
    assert(is_number(c) || !"c must be a  number");
    assert(is_finite(c) || !"c must be a finite number");
    assert(c > 0 || !"c must be positive");

    T const volume = (4 * std::numbers::pi_v<T> * a * b * c) / 3;
    T const mass = density * volume;
    T const five = T(5.0);

    Properties<T> value;

    value.m_m = mass;
    value.m_Ixx = mass * (b * b + c * c) / five;
    value.m_Iyy = mass * (a * a + c * c) / five;
    value.m_Izz = mass * (a * a + b * b) / five;

    return value;
}

template Properties<float> compute_ellipsoid<float>(float const& density, float const& a, float const& b,
                                                    float const& c);

template Properties<double> compute_ellipsoid<double>(double const& density, double const& a, double const& b,
                                                      double const& c);

} // namespace mass
