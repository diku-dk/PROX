...
template <typename T> class BoxMesh : public mass::FaceCallbackInterface<T>
{
public:
    T m_x1[4];
    T m_y1[4];
...
public:
    size_t get_number_of_vertices(size_t const&) const { return 4u; }
...
public:
    BoxMesh() { this->set_box(2.0, 2.0, 2.0, 2.0, 2.0); }

    BoxMesh(T const& top_width, T const& top_depth, T const& bottom_width, T const& bottom_depth, T const& height)
    {
        this->set_box(top_width, top_depth, bottom_width, bottom_depth, height);
    }

protected:
    void set_box(T const& top_width, T const& top_depth, T const& bottom_width, T const& bottom_depth, T const& height)
    {
...
BOOST_AUTO_TEST_CASE(box_mesh)
{
    ...
    mass::Properties<double> Ibox
        = compute_box(rho, 1.0, 1.0, 1.0);  // Compute ground truth value using analytical solution
