...
void make_arch(content::API* engine, typename MT::vector3_type const& position,
               typename MT::quaternion_type const& orientation, typename MT::real_type const& r_outer,
               typename MT::real_type const& r_inner, typename MT::real_type const& pillar_height,
               typename MT::real_type const& stone_depth, size_t const& arch_slices, size_t const& pillar_segments,
               MaterialInfo<typename MT::real_type> mat_info)
{
    ...
    GeometryHandle<MT> pillar_stone
        = create_geometry_handle_box<MT>(engine, pillar_stone_width, pillar_stone_height, stone_depth);
    ...
    for (size_t i = 0; i < pillar_segments; ++i)
    {
        ...
        compute_body_to_world_transform<MT>(T_b2m, Q_b2m, T_left_m2l, Q_left_m2l, T_l2w, Q_l2w, T_left_b2w, Q_left_b2w);

        create_rigid_body<MT>(engine, T_left_b2w, Q_left_b2w, pillar_stone, mid, stone_density);
        ...
        compute_body_to_world_transform<MT>(T_b2m, Q_b2m, T_right_m2l, Q_right_m2l, T_l2w, Q_l2w, T_right_b2w,
                                            Q_right_b2w);

        create_rigid_body<MT>(engine, T_right_b2w, Q_right_b2w, pillar_stone, mid, stone_density);
    }
    ...
    for (size_t i = 0; i < arch_slices; ++i)
    {
        ...
        create_rigid_body<MT>(engine, T_b2w, Q_b2w, arch_stone, mid, stone_density);
    }
}

using MTf = tiny::MathTypes<float>;

template void make_arch<MTf>(content::API* engine, MTf::vector3_type const& position,
                             MTf::quaternion_type const& orientation, MTf::real_type const& r_outer,
                             MTf::real_type const& r_inner, MTf::real_type const& pillar_height,
                             MTf::real_type const& stone_depth, size_t const& arch_slices,
                             size_t const& pillar_segments, MaterialInfo<MTf::real_type> mat_info);
