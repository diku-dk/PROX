                        ...
                        const T& stone_dim, size_t const& layers, const T& degree, MaterialInfo<T> mat_info)
{
    ...
    T twist = degree * (std::numbers::pi_v<T> / 180);
    ;
    for (size_t i = 0u; i < layers; ++i)
    {
        ...
        const EigenQuaternion<T> Q_m2l = Rotatey(twist * i);
...
template void make_twisted_stack<float>(content::API* engine, const EigenVector3<float>& position,
                                        const EigenQuaternion<float>& orientation, const float& stone_dim,
                                        size_t const& layers, const float& degree, MaterialInfo<float> mat_info);
