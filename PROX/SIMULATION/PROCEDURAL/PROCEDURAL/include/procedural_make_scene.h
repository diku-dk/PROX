                       ...
                       util::ConfigFile const& params)
{
    ...
    if (scene.compare("ccd_bunny_bunny") == 0)
    {
            ...
            EigenQuaternion<T>::Identity(), scene_size, mat_info);
        {
            ...
            size_t const rid = create_rigid_body<T>(engine, T_b2w, Q_b2w, bunny,
                                                    mid, stone_density);
            engine->set_rigid_body_velocity(rid, 0.0, 40.0, 5.0);
        }
        {
            GeometryHandleEigen<T> const bunny = create_geometry_handle_obj<T>(
                engine, obj_path + "bunny.obj", 12.0, 12.0, 12.0,
            ...
            const EigenVector3<T> T_m2l = EigenVector3<T>(0, 0.0, 0.0);
            const EigenQuaternion<T> Q_m2l = Rotatey<T>(3.141593);
            ...
            size_t const rid = create_rigid_body<T>(engine, T_b2w, Q_b2w, bunny,
                                                    mid, stone_density);
            engine->set_rigid_body_velocity(rid, 5.0, -40.0, 0.0);
        }
    }
    if (scene.compare("ccd_another_benchmark_test") == 0)
    {
        auto const scene_size = util::to_value<float>(
            params.get_value("procedural_param_1", "10.0"));
        ...
        for (size_t i = 0; i < gridSize; ++i)
        {
            for (size_t j = 0; j < gridSize; ++j)
            {
                ...
                GeometryHandleEigen<T> const bunny
                    = create_geometry_handle_obj<T>(
                        engine, obj_path + "sphere.obj", 0.5, 0.5, 0.5,
                        mesh_array::tetgen_cdt_settings());
                const EigenVector3<T> T_b2m = bunny.Tb2m();
                const EigenQuaternion<T> Q_b2m = bunny.Qb2m();

                const EigenVector3<T> T_m2l = EigenVector3<T>(i, 6.0, j);
                const EigenQuaternion<T> Q_m2l = EigenQuaternion<T>::Identity();
                ...
                size_t const rid = create_rigid_body<T>(
                    engine, T_b2w, Q_b2w, bunny, mid, stone_density);
                engine->set_rigid_body_velocity(rid, 0.0, float(j + 1) * 4.0f,
                                                0.0);
    ...
    if (scene.compare("ccd_another_benchmark_test_two") == 0)
    {
        auto const scene_size = util::to_value<float>(
            params.get_value("procedural_param_1", "10.0"));

        float const ground_width = scene_size * 20.0;
        float const ground_height = scene_size / 10.0;
        float const ground_depth = scene_size * 20.0;
    ...
    if (scene.compare("ccd_spinning_object_shot") == 0)
    {
        ...
        }
        {
            ...
            size_t const rid = create_rigid_body<T>(engine, T_b2w, Q_b2w, bunny,
                                                    mid, stone_density);
            engine->set_rigid_body_velocity(rid, -25.0, 6.0, 0.0);
    ...
    if (scene.compare("ccd_tight_space_drop") == 0)
    {
        ...
        {
            GeometryHandleEigen<T> const bunny = create_geometry_handle_obj<T>(
                engine, obj_path + "sphere.obj", 0.501, 0.501, 0.501,
                mesh_array::tetgen_cdt_settings());
            const EigenVector3<T> T_b2m = bunny.Tb2m();
            const EigenQuaternion<T> Q_b2m = bunny.Qb2m();
    ...
    if (scene.compare("ccd_toi_zero_towards_wall") == 0)
    {
        ...
        procedural::make_ground<T>(engine, EigenVector3<T>(0, -1, 0),
                                   EigenQuaternion<T>::Identity(), mat_info,
                                   ground_width, ground_height, ground_depth);
        ...
        if (true)
        {
            ...
            const EigenVector3<T> T_m2l = EigenVector3<T>(0.75, 0.255, 0.0);
            const EigenQuaternion<T> Q_m2l = EigenQuaternion<T>::Identity();
            ...
            size_t const rid = create_rigid_body<T>(engine, T_b2w, Q_b2w, bunny,
                                                    mid, stone_density);
            engine->set_rigid_body_velocity(rid, -15, 0.0, 0.0);
            //engine->set_rigid_body_fixed(rid, true);
        }
    }
    if (scene.compare("ccd_toi_zero_towards_wall_very_close") == 0)
    {
        ...
        if (true)
        {
            ...
            const EigenVector3<T> T_m2l = EigenVector3<T>(0.24, 0.255, 0.0);
            const EigenQuaternion<T> Q_m2l = EigenQuaternion<T>::Identity();
    ...
    if (scene.compare("ccd_objects_large_spin") == 0)
    {
            ...
            EigenQuaternion<T>::Identity(), scene_size, mat_info);
        {
            ...
            size_t const rid = create_rigid_body<T>(engine, T_b2w, Q_b2w, bunny,
                                                    mid, stone_density);
            engine->set_rigid_body_velocity(rid, 10.0, 6.0, 0.0);
            engine->set_rigid_body_spin(rid, 100.0, 0.0, 0.0);
            //engine->set_rigid_body_fixed(rid, true);
        }
        {
