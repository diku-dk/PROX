#ifndef PROX_SUBSOLVER_BIND_FRICTION_H
...
namespace prox
{
  ...
  inline FrictionSubSolverBinder<T> bind_friction_solver( friction_sub_solver_type const & type )
  {
    ...
    switch( type )
    {
      ...
      case gjk_ellipsoid:
        logging << "bind_friction_solver(): using gjk ellipsoid solver"<< util::Log::newline();
        return FrictionSubSolverBinder<T>( &detail::gjk_ellipsoid<T> );
...
// PROX_SUBSOLVER_BIND_FRICTION_H
#endif
