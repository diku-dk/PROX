...
template <typename T>
T UnsignedDistanceAtTime(const DistanceAtTimeParams<T>& params, T t)
{
    EigenVector3<T> x = BarycentricInterpolate(params.u, params.v, params.w, t);
    return std::abs<T>(grid::value_at_2(params.grid, x));
}

template <typename T>
T SignedDistanceAtTime(const DistanceAtTimeParams<T>& params, T t)
{
    EigenVector3<T> x = BarycentricInterpolate(params.u, params.v, params.w, t);
                ...
                const grid::Grid<T, T>& grid,*/)
{
    ...
    {
        EigenVector3<T> xti = BarycentricInterpolate(u, v, w, ti);
        ...
        EigenVector3<T> xtip1 = BarycentricInterpolate(u, v, w, tip1);
