...
inline bool compute_raycast(geometry::RayEigen<T> const& ray,
                            std::vector<RigidBody<T>>& bodies,
                            narrow::System<T>& narrow_system,
                            size_t& body_index, EigenVector3<T>& point,
                            T& distance)
{
    //--- Initialization -------------------------------------------------------
    point = {0, 0, 0};
    ...
    for (auto geometry = narrow_system.begin(); geometry != narrow_system.end();
         ++geometry)
    {
    ...
    for (auto body = bodies.begin(); body != bodies.end(); ++body, ++idx)
    {
        ...
        if (geometry::compute_raycast_aabb<T>(ray, (aabb), aabb_point,
                                              aabb_distance))
        {
            T body_distance = std::numeric_limits<T>::max();
            EigenVector3<T> body_point(0, 0, 0);

            auto const& geometry
                = narrow_system.get_geometry(body->get_geometry_idx());

            bool body_hit = narrow::raycast(
                ray, *body, geometry, body->get_position(),
                body->get_orientation(), body_point, body_distance);
