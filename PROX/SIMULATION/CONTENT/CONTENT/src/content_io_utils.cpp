...
bool read_collection(TiXmlElement const* tag, std::string const& item, bool (*dispatch)(TiXmlElement const*, Cache&),
                     Cache& data)
{
    TiXmlElement const* child = tag->FirstChildElement(item);
    for (; child; child = child->NextSiblingElement(item))
    {
        if (!(*dispatch)(child, data)) return false;
    }
    return true;
}

bool read_bool(TiXmlElement const* element, std::string const& attrib_name, bool const& default_value,
               bool const& mandatory)
{
    ...
    if (text.size() == 0)
    {
        if (mandatory) throw std::runtime_error(attrib_name + " on " + element->Value() + " did not exist");
        return default_value;
    }

    if (!(text.compare("true") == 0 || text.compare("false") == 0))
    {
        if (mandatory) throw std::runtime_error(attrib_name + " on " + element->Value() + " was  set to " + text);
        return default_value;
    }
    if (text.compare("true") == 0) return true;
    return false;
}
...
Transform read_transform(TiXmlElement const* element, Cache& data, Transform const& default_value)
{
    ...
    if (ref.size() != 0) { value = data.get_transform_from_name(ref); }
...
Motion read_motion(TiXmlElement const* element, Cache& data, Motion const& default_value)
{
    ...
    std::string ref = read_string(element, "ref", false);
    if (ref.size() != 0) { value = data.get_motion_from_name(ref); }
