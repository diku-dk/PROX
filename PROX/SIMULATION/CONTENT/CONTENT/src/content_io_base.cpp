...
float read_float(TiXmlElement const* element, std::string const& attrib_name, float const& default_value,
...
int read_integer(TiXmlElement const* element, std::string const& attrib_name, int const& default_value,
                 bool const& mandatory)
{
...
TiXmlElement const* get_singleton_child(TiXmlNode const* parent, std::string const& name, bool const& mandatory)
{
    ...
    if (!child)
    {
        if (mandatory) throw std::runtime_error(name + " on " + parent->Value() + " did not exist");
        return nullptr;
    }
