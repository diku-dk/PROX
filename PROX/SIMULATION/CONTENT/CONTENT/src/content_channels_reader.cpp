...
bool read_collection(TiXmlElement const* tag, std::string const& item,
                     bool (*dispatch)(TiXmlElement const*, ChannelCache&), ChannelCache& data)
{
...
bool read_channel(TiXmlElement const* tag, ChannelCache& data)
{
    ...
    if (!read_collection(tag, "key", &read_key, data)) return false;
...
bool xml_read(std::string const& filename, ChannelStorage& storage)
{
    ...
    if (!read_collection(channels_tag, "channel", &(details::read_channel), data)) return false;
