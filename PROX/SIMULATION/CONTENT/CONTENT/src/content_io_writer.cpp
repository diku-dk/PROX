...
std::string get_geometry_name(size_t const& idx, Cache& data)
{
    ...
    if (name.size() == 0) { name = "geometry" + util::to_string(idx); }
    return name;
}
...
std::string get_rigid_body_name(size_t const& idx, Cache& data)
{
    ...
    if (name.size() == 0) { name = "body" + util::to_string(idx); }
    return name;
}
...
std::string get_material_name(size_t const& idx, Cache& data)
{
    ...
    if (name.size() == 0) { name = "material" + util::to_string(idx); }
    return name;
}

std::string get_pin_force_name(size_t const& idx, Cache& data) { return "pin_" + util::to_string(idx); }
...
bool make_doc(TiXmlDocument& doc, Cache& data)
{
    ...
    auto* physics = new TiXmlElement("physics");
    doc.LinkEndChild(decl);
    doc.LinkEndChild(physics);
    auto* params = new TiXmlElement("params");
    auto* materials = new TiXmlElement("materials");
    auto* configuration = new TiXmlElement("configuration");
    auto* channels = new TiXmlElement("channels");
      ...
      // add material elements to materials
    {
        ...
        for (size_t i = 0; i < N; ++i)
        {
            ...
            auto* material = new TiXmlElement("material");
      ...
      // add properties to materials
    {
        ...
        for (size_t i = 0; i < N; ++i)
        {
            ...
            auto* friction = new TiXmlElement("friction");
            float mux = 0.0f;
            float muy = 0.0f;
            float muz = 0.0f;
            ...
            auto* direction = new TiXmlElement("direction");
            float dirx = 0.0f;
            float diry = 0.0f;
            float dirz = 0.0f;
    ...
    {
        auto* geometries = new TiXmlElement("geometries");
        ...
        for (size_t i = 0; i < N; ++i)
        {
            ...
            geometry->SetAttribute("type",
                                   "collection"); // 2009-12-29 kenny: currently only collections are supported!
            ...
            for (size_t box_no = 0; box_no < M; ++box_no)
            {
                ...
                auto* transform = new TiXmlElement("transform");
                float x = 0.0f;
                float y = 0.0f;
                float z = 0.0f;
            ...
            for (size_t cap_no = 0; cap_no < M; ++cap_no)
            {
                ...
                auto* capsule = new TiXmlElement("capsule");
            ...
            for (size_t cone_no = 0; cone_no < M; ++cone_no)
            {
                ...
                auto* cone = new TiXmlElement("cone");
            ...
            {
                auto* convex = new TiXmlElement("convex");
                ...
                {
                    auto* point = new TiXmlElement("point");
            ...
            for (size_t cyl_no = 0; cyl_no < M; ++cyl_no)
            {
                ...
                auto* cylinder = new TiXmlElement("cylinder");
            ...
            {
                auto* ellipsoid = new TiXmlElement("ellipsoid");
            ...
            {
                auto* sphere = new TiXmlElement("sphere");
                float radius = 1.0f;
            ...
            for (size_t tetramesh_no = 0; tetramesh_no < M; ++tetramesh_no)
            {
                ...
                auto* tetramesh = new TiXmlElement("tetramesh");
                ...
                {
                    auto* vertex = new TiXmlElement("vertex");
                ...
                {
                    auto* tetrahedron = new TiXmlElement("tetrahedron");
    ...
    {
        auto* forces = new TiXmlElement("forces");
        configuration->LinkEndChild(forces);

        // Add gravity
        auto* gravity = new TiXmlElement("gravity");
        float x = 0.0f;
        float y = 1.0f;
        float z = 0.0f;
        ...
        // Add damping
        auto* damping = new TiXmlElement("damping");
        float linear = 0.01f;
        float angular = 0.01f;
    ...
    {
        auto* scripted_motions = new TiXmlElement("scripted_motions");
        ...
        if (number_of_motions > 0u)
        {
            ...
            for (size_t p = 0u; p < number_of_motions; ++p)
            {
                ...
                {
                    auto* keyframe_motion = new TiXmlElement("keyframe");
                    ...
                    if (numer_of_positions > 0u)
                    {
                        ...
                        data.output()->get_key_frame_positions(motion_idx, &time_array[0], &x_array[0], &y_array[0],
                                                               &z_array[0]);
                    ...
                    if (numer_of_orientations > 0u)
                    {
                        ...
                        data.output()->get_key_frame_orientations(motion_idx, &time_array[0], &qs_array[0],
                                                                  &qx_array[0], &qy_array[0], &qz_array[0]);

                        for (size_t tick = 0u; tick < numer_of_orientations; ++tick)
                        {
                            auto* orientation = new TiXmlElement("orientation");
                ...
                {
                    auto* oscilation_motion = new TiXmlElement("oscilation");
                    ...
                    data.output()->get_scripted_oscilation_paramters(motion_idx, amplitude, frequency, phase, dir_x,
                                                                     dir_y, dir_z);
    ...
    {
        auto* objects = new TiXmlElement("objects");
        ...
        for (size_t i = 0; i < N; ++i)
        {
            ...
            auto* object = new TiXmlElement("object");
            ...
            // add state element
            auto* state = new TiXmlElement("state");
            ...
            auto* motion = new TiXmlElement("motion");
            float vx = 0.0f;
            float vy = 0.0f;
            float vz = 0.0f;
            ...
            auto* shape = new TiXmlElement("shape");
            ...
            if (number_of_forces > 0)
            {
                ...
                for (size_t f = 0u; f < number_of_forces; ++f)
                {
                    ...
                    auto* apply = new TiXmlElement("apply");
...
bool xml_write(std::string const& filename, content::Output* output)
{
    ...
    if (!content::details::make_doc(doc, data)) return false;
