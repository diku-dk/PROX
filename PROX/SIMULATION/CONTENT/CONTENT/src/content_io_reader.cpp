...
bool read_convex(TiXmlElement const* tag, Cache& data)
{
    ...
    data.clear_points();
    if (!read_collection(tag, "point", &read_point, data)) return false;
...
bool read_tetramesh(TiXmlElement const* tag, Cache& data)
{
    ...
    data.clear_vertices();
    if (!read_collection(tag, "vertex", &read_vertex, data)) return false;

    data.clear_tetrahedra();
    if (!read_collection(tag, "tetrahedron", &read_tetrahedron, data)) return false;
...
bool read_geometry(TiXmlElement const* tag, Cache& data)
{
    ...
    if (type.size() == 0 || type.compare("collection") == 0)
    {
        ...
        if (!read_collection(tag, "box", &read_box, data)) return false;
        if (!read_collection(tag, "capsule", &read_capsule, data)) return false;
        if (!read_collection(tag, "cone", &read_cone, data)) return false;
        if (!read_collection(tag, "convex", &read_convex, data)) return false;
        if (!read_collection(tag, "cylinder", &read_cylinder, data)) return false;
        if (!read_collection(tag, "ellipsoid", &read_ellipsoid, data)) return false;
        if (!read_collection(tag, "sphere", &read_sphere, data)) return false;

        TiXmlElement const* tetramesh = get_singleton_child(tag, "tetramesh");
        if (tetramesh)
        {
            if (!read_tetramesh(tetramesh, data)) return false;
        }
...
bool read_property(TiXmlElement const* tag, Cache& data)
{
    ...
    if (first_pos >= last_pos || first_char != '(' || last_char != ')' || comma_pos <= first_pos
        || comma_pos >= last_pos)
    {
    ...
    if (master.size() == 0u)
    {
        throw std::runtime_error("Missing master of the paired materials: " + first_material + " or "
                                 + second_material);
        return false;
    }
    ...
    {
        throw std::runtime_error(master + " was none one of the paired materials: " + first_material + " or "
                                 + second_material);
        return false;
    }
...
RigidBodyState read_state(TiXmlElement const* tag, Cache& data)
{
    ...
    if (ref.size() != 0) { value = data.get_state_from_name(ref); }

    TiXmlElement const* transform = get_singleton_child(tag, "transform", false);
    if (transform) value.m_T = read_transform(transform, data, value.m_T);

    TiXmlElement const* motion = get_singleton_child(tag, "motion", false);
    if (motion) value.m_M = read_motion(motion, data, value.m_M);

    TiXmlElement const* mass = get_singleton_child(tag, "mass", false);
    if (mass) value.m_mass = read_float(mass, "value", value.m_mass, false);
    ...
    if (motion_name.size() > 0u) value.m_motion_idx = data.get_idx_from_name(motion_name);

    bool mandatory = ref.size() == 0;
    std::string material = read_string(tag, "material", mandatory);
    if (material.size() > 0u) value.m_material_idx = data.get_idx_from_name(material);
...
bool read_object(TiXmlElement const* tag, Cache& data)
{
    ...
    if (type.size() == 0 || type.compare("rigid") == 0)
    {
        ...
        data.input()->set_rigid_body_orientation(body_idx, state.m_T.m_qs, state.m_T.m_qx, state.m_T.m_qy,
                                                 state.m_T.m_qz);
        ...
        if (forces)
        {
            if (!read_collection(forces, "apply", &read_apply_force, data)) return false;
        }
...
bool read_forces(TiXmlElement const* tag, Cache& data)
{
    ...
    if (gravity) { read_gravity(gravity, data); }

    TiXmlElement const* damping = get_singleton_child(tag, "damping", false);
    if (damping) { read_damping(damping, data); }
...
bool read_oscilation_scripted_motion(TiXmlElement const* tag, Cache& data)
{
    ...
    data.input()->set_scripted_oscilation_paramters(motion_idx, amplitude, frequency, phase, dir_x, dir_y, dir_z, ref_x,
                                                    ref_y, ref_z);
    return true;
}
...
bool read_physics(TiXmlElement const* tag, Cache& data)
{
    ...
    if (!read_params(get_singleton_child(tag, "params"), data)) return false;
    ...
    if (materials)
    {
        ...
        if (!read_collection(materials, "material", &read_material, data)) return false;

        assert(properties || !"properties tag was missing");

        if (!read_collection(properties, "property", &read_property, data)) return false;
    }

    if (!read_configuration(get_singleton_child(tag, "configuration"), data)) return false;
...
bool xml_read(std::string const& filename, content::Input* input)
{
    ...
    if (!details::read_physics(physics, data)) { return false; }
    return true;
}
