...
public:
  ...
  /**
   * Narrow Phase GeometryEigen Type.
   * The GeometryEigen of an object consists of a collection of shapes. The object defines
   * a local coordinate system in which the shapes live. Ie. each shape has an associated
   * transform that specifies how the shape is placed in the local object frame. When an
   * object is simulated then the simulator essientially moves the local object frame
   * to a new placement. However, the shapes rigidly follows the local object frames as it
   * moves around.
   *
   * This class offers an interface for easy manipulation of the shapes that make up the GeometryEigen
   * of an object.
   *
   * @tparam M     The Math types used.
   */
  template <typename T> class GeometryEigen
  {

  public:
      typedef detail::ShapeTypesEigen<T> shape_types;
  ...
  public:
      ...
      GeometryEigen(GeometryEigen const& geo) { *this = geo; }

      GeometryEigen& operator=(GeometryEigen const& geo)
      {
          if (this != &geo)
          {
              this->m_boxes = geo.m_boxes;
  ...
  public:
     ...
     * Add Shape to geoemtry.
     *
     * @warning   This method will create a copy of the added shape inside the GeometryEigen class.
     *            Thus, subsequent changes to the shape argument instance after invokation
     *            has not effect on the GeometryEigen.
     */
      void add_shape(box_type const& box) { this->add_shape(this->m_boxes, box); }
  ...
  public:
      void update_radius()
      {
          ...
          for (size_t i = 0; i < number_of_hulls(); ++i)
          {
              const CoordSysEigen<T> X = this->m_hulls[i].transform();
