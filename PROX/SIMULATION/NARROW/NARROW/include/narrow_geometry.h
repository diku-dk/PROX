...
public:
    ...
    typedef typename shape_types::SignedDistanceMap signedDistanceMapType;
    ...
    //typedef signedDistanceMap signedDistanceMapContainer;
...
public:
    ...
    Geometry& operator=( Geometry const & geo)
    {
      if( this != &geo)
      {
        ...
        this->m_signedDistanceMap = geo.m_signedDistanceMap;
  ...
  public:
     ...
     */
      void add_shape(box_type const& box)
      {
          this->add_shape(this->m_boxes, box);
      }

      void add_shape(sphere_type const& sphere)
      {
          this->add_shape(this->m_spheres, sphere);
      }

      void add_shape(convex_type const& hull)
      {
          this->add_shape(this->m_hulls, hull);
      }

      void add_shape(tetramesh_type const& mesh) { this->m_tetramesh = mesh; }

      void add_shape(const signedDistanceMapType& sdfMap)
      {
          this->m_signedDistanceMap = sdfMap;
      }
      ...
      size_t number_of_tetrameshes() const
      {
          return this->m_tetramesh.has_data() ? 1 : 0;
      }

      size_t number_of_signedDistanceFields() const
      {
          return this->m_signedDistanceMap.hasData() ? 1 : 0;
      }
      ...
      void clear()
      {
          ...
          this->m_signedDistanceMap.clear();
      }
