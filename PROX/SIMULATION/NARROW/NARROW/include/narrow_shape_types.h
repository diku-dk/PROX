...
namespace narrow
{
  ...
  protected:
    ...
    public:
      ...
      class SignedDistanceMap : public ShapeBase
      {
      protected:
      ...
      public:
          const grid::Grid<T, T>& getSignedDistanceGrid() const
          {
              return m_grid;
          }
          ...
          void clear()
          {
              m_grid.m_min({T(0), T(0), T(0)});
              m_grid.m_max({T(0), T(0), T(0)});
              m_grid.m_dir({T(0), T(0), T(0)});
              m_grid.m_nodes({0u, 0u, 0u});
              m_grid.m_data.resize(0, 0);
          }
      };
