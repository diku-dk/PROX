...
namespace narrow
{
  ...
  )
  {
...
#ifdef USE_PROFILING
    {
        ...
        err = queue.finish();
        CHECK_CL_ERR(err);
        cl_ulong tv_start = tv_profiling.getProfilingInfo<CL_PROFILING_COMMAND_START>(&err);
        CHECK_CL_ERR(err);
        cl_ulong tv_end = tv_profiling.getProfilingInfo<CL_PROFILING_COMMAND_END>(&err);
        CHECK_CL_ERR(err);
        refit_tree_kernel_time = tv_end - tv_start;
    }
#else
    err = queue.enqueueNDRangeKernel(  transform_vertices_kernel
                                     , ::cl::NullRange
