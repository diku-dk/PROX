...
BOOST_AUTO_TEST_CASE(grid_test_grid)
{
...

    {
        ...
        grid::Grid<D, T> G;
        G.create(Eigen::Matrix<T, 3, 1>(0, 0, 0), Eigen::Matrix<T, 3, 1>((T)(nx - 1), (T)(ny - 1), (T)(nz - 1)),
                 Eigen::Matrix<size_t, 3, 1>(nx, ny, nz));

        const size_t total = G.m_nodes.x() * G.m_nodes.y() * G.m_nodes.z();

        //Fill with linear data
        for (size_t s = 0; s < total; ++s) { G.data()[s] = static_cast<D>(s + 100); }
        ...
        if (ok) { std::cout << "Iterator forward traversal: PASS\n"; }
        else { std::cout << "Iterator forward traversal: FAIL\n"; }
        ...
        if (lin != 5)
        {
            std::cout << "compute_index mismatch: got linear=" << lin << " expected 5 -> idx(" << idx.x() << ","
                      << idx.y() << "," << idx.z() << ")\n";
        }
        else
        {
        ...
        else { std::cout << "IndexIterator assignment: PASS (value=" << *iit << ")\n"; }
        ...
        ++iit;
        if (iit.i() != i_before + 1) { std::cout << "IndexIterator ++ failed to increment i.\n"; }
        ...
        if (iit.i() != 0 || iit.j() != 1)
        {
            std::cout << "IndexIterator ++ failed to wrap row to next column. got (" << iit.i() << "," << iit.j() << ","
                      << iit.k() << ")\n";
        }
        else { std::cout << "IndexIterator increment behavior: PASS\n"; }
        ...
        for (size_t k = 0; k < res; ++k)
        {
            for (size_t j = 0; j < res; ++j)
            {
                for (size_t i = 0; i < res; ++i)
                {
                    ...
                    if (i + 1 < res)
                    {
                        if (v * G(Eigen::Matrix<size_t, 3, 1>(i + 1, j, k)) < 0.0) ++signEdges;
                    }
                    if (j + 1 < res)
                    {
                        if (v * G(Eigen::Matrix<size_t, 3, 1>(i, j + 1, k)) < 0.0) ++signEdges;
                    }
                    if (k + 1 < res)
                    {
                        if (v * G(Eigen::Matrix<size_t, 3, 1>(i, j, k + 1)) < 0.0) ++signEdges;
                    }
                }
            }
        }

        std::cout << "Sign-change edges count: " << signEdges
                  << " (should be > 0 if the surface intersects the grid)\n";
        if (signEdges == 0)
        {
            //std::cout << "ERROR!";
        }
...

    {
        ...
        Eigen::Matrix<T, 3, 1> gmin((T)(minv.x() - pad), (T)(minv.y() - pad), (T)(minv.z() - pad));
        Eigen::Matrix<T, 3, 1> gmax((T)(maxv.x() + pad), (T)(maxv.y() + pad), (T)(maxv.z() + pad));
        ...
        G.create(gmin, gmax, nodes);
        const size_t total = G.m_nodes.x() * G.m_nodes.y() * G.m_nodes.z();
        std::cout << "Created grid: " << G.I() << " x " << G.J() << " x " << G.K() << "  (total nodes = " << total
                  << ")\n";
        ...
        {
            std::cerr << "ERROR: libigl returned unexpected S size: " << S.size() << " expected " << total << "\n";
        }

        //Copy into grid storage
        for (size_t s = 0; s < total; ++s) { G.data()[s] = static_cast<D>(S((Eigen::Index)s)); }
        ...
        if (!csv) { std::cerr << "ERROR: cannot open " << outCsv << " for writing\n"; }
        csv << "x,y,z,sdf\n";
        idx_lin = 0;
        for (size_t k = 0; k < G.K(); ++k)
        {
            for (size_t j = 0; j < G.J(); ++j)
            {
                for (size_t i = 0; i < G.I(); ++i)
                {
                    ...
                    Eigen::Matrix<T, 3, 1> p;
                    grid::node_position(G, idx, p);
                    csv << std::setprecision(9) << p.x() << "," << p.y() << "," << p.z() << "," << G(idx) << "\n";
                    ++idx_lin;
                }
            }
        }
        ...
        double minS = S.minCoeff();
        double maxS = S.maxCoeff();
        std::cout << std::fixed << std::setprecision(6) << "Signed distance range: [" << minS << ", " << maxS << "]\n";
    }
}
