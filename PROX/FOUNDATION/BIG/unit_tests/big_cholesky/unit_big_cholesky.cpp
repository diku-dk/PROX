...
*/
template < class MATRIX > void fill_symm(MATRIX& L, const size_t bands = std::numeric_limits<size_t>::max())
{
...
void test(matrix_type const& A, vector_type& x, vector_type const& b, vector_type const& y)
{
  ...
  // Dense versions
    big::cholesky_decompose(A, L);
    x = b;
    big::cholesky_solve(L, x, ublas::lower());
    for (size_type i = 0; i < x.size(); ++i) BOOST_CHECK_CLOSE(real_type(x(i)), real_type(y(i)), tol);
    x.clear();

    L.assign(A);
    big::cholesky_decompose(L);
    x = b;
    big::cholesky_solve(L, x, ublas::lower());
    for (size_type i = 0; i < x.size(); ++i) BOOST_CHECK_CLOSE(real_type(x(i)), real_type(y(i)), tol);
    x.clear();
  ...
  // sparse version
    ...
    C.resize(m, n, false);
    C.assign(A);
    big::incomplete_cholesky_decompose(C);
    x = b;
    big::cholesky_solve(C, x, ublas::lower());
    for (size_type i = 0; i < x.size(); ++i) BOOST_CHECK_CLOSE(real_type(x(i)), real_type(y(i)), tol);
    x.clear();

  // Easy sparse version
    C.assign(A);
    big::cholesky_solve(C, x, b);
    for (size_type i = 0; i < x.size(); ++i) BOOST_CHECK_CLOSE(real_type(x(i)), real_type(y(i)), tol);
}

BOOST_AUTO_TEST_SUITE(big_cholesky);
...
BOOST_AUTO_TEST_CASE(Gunter_Winkler_Konstantin_Kutzkow_Test)
{
    ...
    // use dense matrix
        ...
        b = prod(A, y);
        size_t res = big::cholesky_decompose(A, L);
