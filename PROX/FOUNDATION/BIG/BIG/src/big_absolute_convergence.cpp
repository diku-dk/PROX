...
namespace big
{
template < typename T > bool absolute_convergence(T const& f, T const& tolerance)
{
    ...
    if (f <= tolerance) return true;

    return false;
}

template bool absolute_convergence<float>(float const& f, float const& tolerance);

template bool absolute_convergence<double>(double const& f, double const& tolerance);
