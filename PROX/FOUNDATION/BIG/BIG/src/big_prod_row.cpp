...
template <typename T>
T prod_row(boost::numeric::ublas::compressed_matrix<T> const& A, boost::numeric::ublas::vector<T> const& x,
           typename boost::numeric::ublas::vector<T>::size_type i)
{
    ...
    if (i >= row_end) return value;

    size_type const begin = A.index1_data()[i];
    size_type const end = A.index1_data()[i + 1];
    for (size_type j = begin; j < end; ++j) value += A.value_data()[j] * x(A.index2_data()[j]);
...
template float prod_row<float>(boost::numeric::ublas::compressed_matrix<float> const& A,
                               boost::numeric::ublas::vector<float> const& x,
                               boost::numeric::ublas::vector<float>::size_type i);

template double prod_row<double>(boost::numeric::ublas::compressed_matrix<double> const& A,
