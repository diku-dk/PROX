...
{
template <typename T>
void prod_trans(boost::numeric::ublas::compressed_matrix<T> const& A, boost::numeric::ublas::vector<T> const& x,
                boost::numeric::ublas::vector<T>& y)
{
    ...
    for (size_type i = 0u; i < row_end; ++i)
    {
        ...
        for (size_type j = begin; j < end; ++j) y(A.index2_data()[j]) += A.value_data()[j] * x(i);
    }
}

template void prod_trans<float>(boost::numeric::ublas::compressed_matrix<float> const& A,
                                boost::numeric::ublas::vector<float> const& x, boost::numeric::ublas::vector<float>& y);

template void prod_trans<double>(boost::numeric::ublas::compressed_matrix<double> const& A,
                                 boost::numeric::ublas::vector<double> const& x,
                                 boost::numeric::ublas::vector<double>& y);

template <typename T>
void prod_trans(boost::numeric::ublas::compressed_matrix<T> const& A, boost::numeric::ublas::vector<T> const& x,
                boost::numeric::ublas::vector<T> const& b, boost::numeric::ublas::vector<T>& y)
{
    typedef boost::numeric::ublas::vector<T> vector_type;
    typedef typename vector_type::size_type size_type;
...
template void prod_trans<float>(boost::numeric::ublas::compressed_matrix<float> const& A,
                                ...
                                boost::numeric::ublas::vector<float> const& b, boost::numeric::ublas::vector<float>& y);
