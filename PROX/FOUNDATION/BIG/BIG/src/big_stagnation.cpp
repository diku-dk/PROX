...
template < typename T > bool stagnation(ublas::vector<T> const& x_old, ublas::vector<T> const& x, T const& tolerance)
{
    using std::fabs;
    using std::max;
    ...
    for (size_t i = 0u; i < m; ++i) max_dx = max(max_dx, fabs(x(i) - x_old(i)));

    assert(is_number(max_dx) || !"stagnation(): internal error, NAN is encountered?");

    if (max_dx <= tolerance) return true;

    return false;
}

template bool stagnation<float>(ublas::vector<float> const& x_old, ublas::vector<float> const& x,
                                float const& tolerance);

template bool stagnation<double>(ublas::vector<double> const& x_old, ublas::vector<double> const& x,
