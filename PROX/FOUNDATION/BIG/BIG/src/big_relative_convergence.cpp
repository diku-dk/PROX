...
template < typename T > bool relative_convergence(T const& f_old, T const& f, T const& tolerance)
{
    using std::fabs;
    ...
    if (relative_test <= tolerance) return true;

    return false;
}

template bool relative_convergence<float>(float const& f_old, float const& f, float const& tolerance);

template bool relative_convergence<double>(double const& f_old, double const& f, double const& tolerance);
