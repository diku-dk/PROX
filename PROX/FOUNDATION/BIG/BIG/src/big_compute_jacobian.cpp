...
template <typename T>
void compute_jacobian(ublas::compressed_matrix<T> const& A, ublas::vector<size_t> const& bitmask,
                      ublas::compressed_matrix<T>& J)
{
    ...
    {
        if (bitmask(i) == IN_NON_ACTIVE) { J(i, i) = 1; }
...
template void compute_jacobian<float>(ublas::compressed_matrix<float> const& A, ublas::vector<size_t> const& bitmask,
                                      ublas::compressed_matrix<float>& J);

template void compute_jacobian<double>(ublas::compressed_matrix<double> const& A, ublas::vector<size_t> const& bitmask,
