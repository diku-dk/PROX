...
inline void test(matrix_type const& A, vector_type& x, vector_type const& b, vector_type const& y, size_t* cnt_status)
{
  ...
  //
    big::projected_gauss_seidel(
        ...
        ,
        tol_abs, tol_rel, tol_stag, status, iter, err);

    big::min_map_newton(A, b, x, max_iter, tol_abs, tol_rel, tol_stag, status, iter, err, alpha, beta);

    cnt_status[status]++;

    std::cout << "error = " << err << " found in " << iter << " Newton iterations "
              << " with final solver status= " << big::get_error_message(status) << std::endl;
}
...
int main()
{
    ...
    size_type N
        = 100; // 2012-09-30 Kenny: right now we use SVD as subsolver in Newton method -- higher dimenions than this kind of removes the interactive aspect of this demo... when we replace SVD with a proper iterative method we should be able to scale to much higher dimensions.
    ...
    for (size_t i = 0; i < 9; ++i) { std::cout << cnt_status[i] << "\t:\t" << big::get_error_message(i) << std::endl; }
    std::cout << std::endl;
