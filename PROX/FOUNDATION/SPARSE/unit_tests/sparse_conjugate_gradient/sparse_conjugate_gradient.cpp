...
BOOST_AUTO_TEST_CASE(sparse_conjugate_gradient_test)
{
  ...
  //test values
    for (size_t i = 0; i < A.size(); ++i) { BOOST_CHECK(x(i) == expected_x(i)); }
  ...
  //is ensured to have full rank
    ...
    {
        for (size_t j = 0; j < 4; ++j) { sparse::mul(1.0f, block3x3_type::identity(), A(i, j)); }
        expected_x(i) = 2;
    }
  //calculate b
  ...
  //run conjugate gradient
    conjugate_gradient(A, x, b);
  //test values
    for (size_t i = 0; i < A.nrows(); ++i) { BOOST_CHECK(x(i) == expected_x(i)); }
  //reset the values for use
    A.clear();
    b.clear_data();
    x.clear_data();
  ...
  //reset the values for use
    ...
    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t j = 0; j < 4; ++j) { sparse::mul(2.0f, block3x3_type::identity(), A(i, j)); }
        expected_x(i) = 2;
    }
