...
void Verify(op_type _result, real_type value)
{
    ...
    for (size_t i = 0; i < stride; ++i) { BOOST_CHECK_CLOSE(result[i], value, tol); }
}

template <typename type_traits> void test_traits()
{
  ...
  //real_type const sixteen	= eight+eight;
    ...
    real_type const _true = 1;
    real_type const _false = 0;
...
//	  break;
    case 1 :  BOOST_CHECK_CLOSE(type_traits::sum(_one), one, tol); break;
    default : break;
    }
...
//	  break;
    case 1 :  BOOST_CHECK_CLOSE(type_traits::mul(_two), two, tol); break;
    default : break;
    }
    ...
    for (size_t i = 0; i < size; ++i) { scalars[i] = tiny::detail::numeric_cast<real_type>(i + 1); }
