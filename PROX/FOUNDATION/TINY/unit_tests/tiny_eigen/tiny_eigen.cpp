...
inline void eigen_value_decomposition_test(vector3_type d, matrix3x3_type R)
{
  ...
  //--- The check close version behaves strange when rhs is exactly zero?, so we use the check-version instead...
    ...
    bool match1 = fabs(d0 - d(0)) < epsilon && fabs(d1 - d(1)) < epsilon && fabs(d2 - d(2)) < epsilon;

    bool match2 = fabs(d0 - d(0)) < epsilon && fabs(d1 - d(2)) < epsilon && fabs(d2 - d(1)) < epsilon;

    bool match3 = fabs(d0 - d(1)) < epsilon && fabs(d1 - d(0)) < epsilon && fabs(d2 - d(2)) < epsilon;

    bool match4 = fabs(d0 - d(1)) < epsilon && fabs(d1 - d(2)) < epsilon && fabs(d2 - d(0)) < epsilon;

    bool match5 = fabs(d0 - d(2)) < epsilon && fabs(d1 - d(0)) < epsilon && fabs(d2 - d(1)) < epsilon;

    bool match6 = fabs(d0 - d(2)) < epsilon && fabs(d1 - d(1)) < epsilon && fabs(d2 - d(0)) < epsilon;
