...
template <typename T> const T& Cylinder<T>::half_height() const { return this->m_half_height; }

template <typename T> T& Cylinder<T>::half_height() { return this->m_half_height; }

template <typename T> const T& Cylinder<T>::radius() const { return this->m_radius; }

template <typename T> T& Cylinder<T>::radius() { return this->m_radius; }
...
template <typename T> auto Cylinder<T>::get_support_point(EigenVector3<T> dir) const -> EigenVector3<T>
{
    auto height = dir.z() > 0 ? m_half_height : -m_half_height;
    auto sigmaNorm = std::sqrt(dir.x() * dir.x() + dir.y() * dir.y());
    if (sigmaNorm > 0) { return {m_radius * dir.x() / sigmaNorm, m_radius * dir.y() / sigmaNorm, height}; }
    return {0, 0, height};
}

template <typename T> T Cylinder<T>::get_scale() const
{
    using std::min;
