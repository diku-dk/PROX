...
namespace convex
{
  ...
  template<typename T>
  const T& Capsule<T>::half_height() const { return this->m_half_height; }

  template<typename T>
  T& Capsule<T>::half_height() { return this->m_half_height; }

  template<typename T>
  const T& Capsule<T>::radius() const { return this->m_radius; }

  template<typename T>
  T& Capsule<T>::radius() { return this->m_radius; }
  ...
  template<typename T>
  auto Capsule<T>::get_support_point(EigenVector3<T> dir) const -> EigenVector3<T>
  {
      using std::sqrt;
