...
/**
     ...
     */
template <typename T>
inline bool extract_feature_from_bitmask(int const& bitmask, std::vector<EigenVector3<T>> const& corners,
...
template <typename T>
inline unsigned int intersect_line_circle(const EigenVector3<T>& p0, const EigenVector3<T>& p1, const T& radius,
                                          EigenVector3<T>& i0, EigenVector3<T>& i1)
{
    using std::fabs;
    using std::sqrt;
...
/**
     ...
     */
template <typename T> inline bool inside_quad(const EigenVector3<T>& p, std::vector<EigenVector3<T>>& quad)
{
...
/**
     ...
     */
template <typename T>
inline void intersect_polygon_circle(std::vector<EigenVector3<T>>& feature, const T& radius,
                                     std::vector<EigenVector3<T>>& intersection)
{
    using std::cos;
    using std::sin;
    ...
    if (feature.size() == 2u)
    {
        ...
        if (!inside0 && !inside1)
        {
            ...
            if (cnt == 1u) { intersection.push_back(p0); }
        }
    ...
    if (feature.size() == 4u)
    {
        ...
        if (inside0) intersection.push_back(feature[0]);
        if (inside1) intersection.push_back(feature[1]);
        if (inside2) intersection.push_back(feature[2]);
        if (inside3) intersection.push_back(feature[3]);
        ...
        for (unsigned int k = 0u; k < 4u; ++k)
        {
            ...
            unsigned int const cnt = intersect_line_circle(feature[k], feature[(k + 1) % 4], radius, p0, p1);
        ...
        for (unsigned int sample = 0u; sample < max_samples; ++sample)
        {
            ...
            if (inside_quad(p, feature)) intersection.push_back(p);
...
/**
   ...
   */
template <typename T>
inline bool contacts_obb_cylinder(OBB<T> const& A, Cylinder<EigenVector3<T>> const& B, T const& envelope,
                                  ContactsCallback<T>& callback, bool const flip = false)
{
    ...
    T const upper_envelope
        = half_height
        - envelope; // "Fuzzy" zone of upper cap plane to counter finite precision problems and truncation errors
    T const lower_envelope
        = -half_height
        + envelope; // "Fuzzy" zone of lower cap plane to counter finite precision problems and truncation errors
    ...
    if (cnt_above == 8u) return false;

    if (cnt_below == 8u) return false;

    //--- Test for special cases of OBB touching the face-caps of the cylinder
    int const top_bit_mask = on_top_cap[0] | on_top_cap[1] << 1 | on_top_cap[2] << 2 | on_top_cap[3] << 3
                           | on_top_cap[4] << 4 | on_top_cap[5] << 5 | on_top_cap[6] << 6 | on_top_cap[7] << 7;

    int const bottom_bit_mask = on_bottom_cap[0] | on_bottom_cap[1] << 1 | on_bottom_cap[2] << 2 | on_bottom_cap[3] << 3
                              | on_bottom_cap[4] << 4 | on_bottom_cap[5] << 5 | on_bottom_cap[6] << 6
                              | on_bottom_cap[7] << 7;
    ...
    if (top_bit_mask != 0 && !cnt_below && !inbeween)
    {
        ...
        if (detail::extract_feature_from_bitmask(top_bit_mask, b, feature))
        {
            ...
            for (typename std::vector<EigenVector3<T>>::const_iterator p = intersection_points.begin();
                 p != intersection_points.end(); ++p)
            {
