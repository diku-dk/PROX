...
AdjacencyInfo<T4Mesh>::~AdjacencyInfo() { this->release(); }

AdjacencyInfo<T4Mesh>::AdjacencyInfo(AdjacencyInfo<T4Mesh> const& A) { *this = A; }
...
AdjacencyInfo<T4Mesh>::AdjacencyInfo(VertexRing<T4Mesh > const& VR) { this->bind(VR); }
    ...
    for (size_t t = 0u; t < T; ++t)
    {
        ...
        while (ci())
        {
          ...
          // Opposite m -- shared face is ijk
                if (ci->has_vertex(j) && ci->has_vertex(k)) { this->m_info[idx].m_m = ci->idx(); }
          // Opposite k -- shared face is ijm
                if (ci->has_vertex(j) && ci->has_vertex(m)) { this->m_info[idx].m_k = ci->idx(); }
          // Opposite j -- shared face is ikm
                if (ci->has_vertex(k) && ci->has_vertex(m)) { this->m_info[idx].m_j = ci->idx(); }
            }
            ++ci;
        }
        ...
        while (cj())
        {
          ...
          // Opposite i -- shared face is jkm
                if (cj->has_vertex(k) && cj->has_vertex(m)) { this->m_info[idx].m_i = cj->idx(); }
            }
            ++cj;
        }
    }
...
size_t AdjacencyInfo<T4Mesh>::i(size_t const& idx) const { return this->m_info[idx].m_i; }

size_t AdjacencyInfo<T4Mesh>::j(size_t const& idx) const { return this->m_info[idx].m_j; }

size_t AdjacencyInfo<T4Mesh>::k(size_t const& idx) const { return this->m_info[idx].m_k; }

size_t AdjacencyInfo<T4Mesh>::m(size_t const& idx) const { return this->m_info[idx].m_m; }
