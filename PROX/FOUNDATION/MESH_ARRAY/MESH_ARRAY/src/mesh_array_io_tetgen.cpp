...
template <typename T>
void read_tetgen(std::string const& filename, T3Mesh& mesh, VertexAttribute<T, T3Mesh>& X,
...
template void read_tetgen<float>(std::string const& filename, T3Mesh& mesh, VertexAttribute<float, T3Mesh>& X,
                                 VertexAttribute<float, T3Mesh>& Y, VertexAttribute<float, T3Mesh>& Z);

template void read_tetgen<double>(std::string const& filename, T3Mesh& mesh, VertexAttribute<double, T3Mesh>& X,
                                  VertexAttribute<double, T3Mesh>& Y, VertexAttribute<double, T3Mesh>& Z);

template <typename T>
void read_tetgen(std::string const& filename, T4Mesh& mesh, VertexAttribute<T, T4Mesh>& X,
                 VertexAttribute<T, T4Mesh>& Y, VertexAttribute<T, T4Mesh>& Z)
{
    ...
    for (size_t idx = 0u; idx < cnt_tetrahedra; ++idx)
    {
        ...
        mesh.push_tetrahedron(vertices[i], vertices[j], vertices[k], vertices[m]);
    }
...
template void read_tetgen<float>(std::string const& filename, T4Mesh& mesh, VertexAttribute<float, T4Mesh>& X,
                                 VertexAttribute<float, T4Mesh>& Y, VertexAttribute<float, T4Mesh>& Z);

template void read_tetgen<double>(std::string const& filename, T4Mesh& mesh, VertexAttribute<double, T4Mesh>& X,
                                  VertexAttribute<double, T4Mesh>& Y, VertexAttribute<double, T4Mesh>& Z);

template <typename T>
void write_tetgen(std::string const& filename, T3Mesh const& mesh, VertexAttribute<T, T3Mesh> const& X,
                  VertexAttribute<T, T3Mesh> const& Y, VertexAttribute<T, T3Mesh> const& Z)
{
    boost::filesystem::path mypath(filename);
    ...
    file1 << cnt_nodes << " " << 3u << " " << 0u << " " << 0u << std::endl;

    file2 << cnt_triangles << " " << 3u << " " << 0u << std::endl;

    for (size_t idx = 0u; idx < cnt_nodes; ++idx)
    {
        Vertex v = mesh.vertex(idx);

        file1 << "  " << v.idx() << "  " << X(v) << " " << Y(v) << " " << Z(v) << std::endl;
    }
    ...
    for (size_t idx = 0u; idx < cnt_triangles; ++idx)
    {
        ...
        file2 << t.idx() << "  " << t.i() << " " << t.j() << " " << t.k() << std::endl;
    }
...
template void write_tetgen<float>(std::string const& filename, T3Mesh const& mesh,
                                  VertexAttribute<float, T3Mesh> const& X, VertexAttribute<float, T3Mesh> const& Y,
                                  VertexAttribute<float, T3Mesh> const& Z);

template void write_tetgen<double>(std::string const& filename, T3Mesh const& mesh,
                                   VertexAttribute<double, T3Mesh> const& X, VertexAttribute<double, T3Mesh> const& Y,
                                   VertexAttribute<double, T3Mesh> const& Z);

template <typename T>
void write_tetgen(std::string const& filename, T4Mesh const& mesh, VertexAttribute<T, T4Mesh> const& X,
                  VertexAttribute<T, T4Mesh> const& Y, VertexAttribute<T, T4Mesh> const& Z)
{
    boost::filesystem::path mypath(filename);
    ...
    file2 << cnt_tetrahedron << " " << 4u << " " << 0u << std::endl;
    ...
    for (size_t idx = 0u; idx < cnt_tetrahedron; ++idx)
    {
        ...
        file2 << t.idx() << "  " << t.i() << " " << t.j() << " " << t.k() << " " << t.m() << std::endl;
    }
...
template void write_tetgen<float>(std::string const& filename, T4Mesh const& mesh,
                                  VertexAttribute<float, T4Mesh> const& X, VertexAttribute<float, T4Mesh> const& Y,
                                  VertexAttribute<float, T4Mesh> const& Z);

template void write_tetgen<double>(std::string const& filename, T4Mesh const& mesh,
                                   VertexAttribute<double, T4Mesh> const& X, VertexAttribute<double, T4Mesh> const& Y,
