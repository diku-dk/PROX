...
std::string TetGenSettings::cmdline() const
{
    ...
    if (m_suppress_splitting) { input << "Y"; }

      //input << "i";

    if (m_verify_input) { input << "d"; }
    else
    {
        if (m_quality_ratio > 0) input << "q" << m_quality_ratio;  // quality

        if (m_maximum_volume > 0) input << "a" << m_maximum_volume;  // max volume

        if (m_quiet_output) input << "Q";  // keep quiet :)
    }
    return input.str();
}

TetGenSettings tetgen_default_settings() { return TetGenSettings(); }
...
template <typename T>
void tetgen(T3Mesh const& surface, VertexAttribute<T, T3Mesh> const& inX, VertexAttribute<T, T3Mesh> const& inY,
            VertexAttribute<T, T3Mesh> const& inZ, T4Mesh& volume, VertexAttribute<T, T4Mesh>& outX,
            VertexAttribute<T, T4Mesh>& outY, VertexAttribute<T, T4Mesh>& outZ, TetGenSettings const& settings)
{
...
template void tetgen<float>(T3Mesh const& surface, VertexAttribute<float, T3Mesh> const& inX,
                            VertexAttribute<float, T3Mesh> const& inY, VertexAttribute<float, T3Mesh> const& inZ,
                            T4Mesh& volume, VertexAttribute<float, T4Mesh>& outX, VertexAttribute<float, T4Mesh>& outY,
                            VertexAttribute<float, T4Mesh>& outZ, TetGenSettings const& settings = TetGenSettings());

template void tetgen<double>(T3Mesh const& surface, VertexAttribute<double, T3Mesh> const& inX,
                             VertexAttribute<double, T3Mesh> const& inY, VertexAttribute<double, T3Mesh> const& inZ,
                             T4Mesh& volume, VertexAttribute<double, T4Mesh>& outX,
                             VertexAttribute<double, T4Mesh>& outY, VertexAttribute<double, T4Mesh>& outZ,
