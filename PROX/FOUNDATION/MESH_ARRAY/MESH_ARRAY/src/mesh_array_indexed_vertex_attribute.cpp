...
template <typename V, typename M> M const* IndexedVertexAttribute<V, M>::mesh() const { return this->m_data.mesh(); }

template <typename V, typename M> void IndexedVertexAttribute<V, M>::bind(M const& m) { this->m_data.bind(m); }

template <typename V, typename M> void IndexedVertexAttribute<V, M>::release() { this->m_data.release(); }

template <typename V, typename M> size_t IndexedVertexAttribute<V, M>::size() const { return this->m_data.size(); }
...
template <typename V, typename M> typename V::real_type& IndexedVertexAttribute<V, M>::operator()(Vertex const& v)
{
    return this->m_data(v)[this->m_idx];
}
