...
AdjacencyInfo<T3Mesh>::~AdjacencyInfo() { this->release(); }

AdjacencyInfo<T3Mesh>::AdjacencyInfo(AdjacencyInfo<T3Mesh> const& A) { *this = A; }
...
AdjacencyInfo<T3Mesh>::AdjacencyInfo(VertexRing<T3Mesh > const& VR) { this->bind(VR); }
    ...
    for (size_t t = 0u; t < T; ++t)
    {
        ...
        while (ci())
        {
          ...
          // Opposite k -- shared face is ij
                if (ci->has_vertex(j)) { this->m_info[idx].m_k = ci->idx(); }

          // Opposite j -- shared face is ik
                if (ci->has_vertex(k)) { this->m_info[idx].m_j = ci->idx(); }
            }
            ++ci;
        }
        ...
        while (cj())
        {
          ...
          // Opposite i -- shared face is jk
                if (cj->has_vertex(k)) { this->m_info[idx].m_i = cj->idx(); }
            }
            ++cj;
        }
    }
...
size_t AdjacencyInfo<T3Mesh>::i(size_t const& idx) const { return this->m_info[idx].m_i; }

size_t AdjacencyInfo<T3Mesh>::j(size_t const& idx) const { return this->m_info[idx].m_j; }

size_t AdjacencyInfo<T3Mesh>::k(size_t const& idx) const { return this->m_info[idx].m_k; }
