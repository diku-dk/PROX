...
VertexRing<T4Mesh>::~VertexRing() { this->release(); }

VertexRing<T4Mesh>::VertexRing(T4Mesh const& m) { this->bind(m); }

VertexRing<T4Mesh>::VertexRing(VertexRing< T4Mesh > const& o) { (*this) = o; }
...
T4Mesh const* VertexRing<T4Mesh>::mesh() const { return this->m_mesh; }
...
void VertexRing<T4Mesh>::bind(T4Mesh const& m)
{
    ...
    for (size_t n = 1u; n < N; ++n)
    {
        ...
        if (prev != cur) { this->m_offset[cur] = n; }
    }

    for (size_t v = 1u; v < V + 1u; ++v)
    {
        size_t const cur = this->m_offset[v];
        if (cur == UNASSIGNED()) continue;
...
std::vector< size_t > const& VertexRing<T4Mesh>::offset() const { return this->m_offset; }

std::vector< std::pair<size_t, size_t > > const& VertexRing<T4Mesh>::V2T() const { return this->m_V2T; }
