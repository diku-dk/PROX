...
template <typename T>
void read_obj(std::string const& filename, T3Mesh& mesh, VertexAttribute<T, T3Mesh>& X, VertexAttribute<T, T3Mesh>& Y,
    ...
    while (!file.eof())
    {
        ...
        //---  f v0_idx//vn0_idx//vt0_idx  ...  vN_idx//vnN_idx//vtN_idx
          ...
          // NOTE: hd 20060605 - hack to let linux version gracefully read windows-formatted files.
                ...
                boost::algorithm::split(split_vector, raw, boost::algorithm::is_any_of("/"),
            ...
            {
                size_t const i
                    = face[0u] - 1u; // obj files uses one-based indexing so we convert to zero-based indexing
...
template void read_obj<float>(std::string const& filename, T3Mesh& mesh, VertexAttribute<float, T3Mesh>& X,
                              VertexAttribute<float, T3Mesh>& Y, VertexAttribute<float, T3Mesh>& Z);

template void read_obj<double>(std::string const& filename, T3Mesh& mesh, VertexAttribute<double, T3Mesh>& X,
                               VertexAttribute<double, T3Mesh>& Y, VertexAttribute<double, T3Mesh>& Z);

template <typename T>
void write_obj(std::string const& filename, T3Mesh const& mesh, VertexAttribute<T, T3Mesh> const& X,
...
template void write_obj<float>(std::string const& filename, T3Mesh const& mesh, VertexAttribute<float, T3Mesh> const& X,
                               VertexAttribute<float, T3Mesh> const& Y, VertexAttribute<float, T3Mesh> const& Z);

template void write_obj<double>(std::string const& filename, T3Mesh const& mesh,
