#ifndef HYPER_COMPUTE_DAMPING_MATRIX_H
#define HYPER_COMPUTE_DAMPING_MATRIX_H
...
namespace hyper
{
  ...
  inline void compute_damping_matrix(
                                  mesh_array::T4Mesh  const & mesh
                                  , typename MT::vector_block3x1_type const & x
                                  , ConstitutiveEquation<MT> const * model
                                  , std::vector< typename MT::element_matrices_type > & Ce
                                  )
  {
    assert( model || !"compute_damping_matrix(): model was NULL");
    ...
    else
    {
      ...
      //
      //  Me = c V / 20  *  [ 2*I_3x3    I_3x3   I_3x3   I_3x3;
